...
module.exports = function()
{
    ...
    PG.setAssetPackWeights = function(assetPacks)
    {
        let weight = 1;
        ...
        for (let i = 0; i < assetPacks.length; i++)
        {
            ...
            for (let j = 0; j < assetPacks[i].flattenedSubgroups.length; j++)
            {
                ...
                {
                    weight *= assetPacks[i].flattenedSubgroups[j][k].probabilityWeighting;
                }
            }
            assetPacks[i].probabilityWeighting = weight;
        }
    }
...
function filterSubgroupsByForceIf(assetPacks, NPCrecordHandle, verboseLog_ForceIfList, out_writeVerbose, logMessage, xelib, attributeCache)
{
    ...
    Aux.logVerbose("Matched forceIf attributes by config file and subgroup: ", verboseLog_ForceIfList, out_writeVerbose);
    for (let i = 0; i < assetPacks.length; i++)
    {
        ...
        for (let j = 0; j < assetPacks[i].flattenedSubgroups.length; j++)
        {
            ...
            for (let k = 0; k < assetPacks[i].flattenedSubgroups[j].length; k++)
            {
                Aux.logVerbose("\t" + assetPacks[i].flattenedSubgroups[j][k].id + ": " + matchCountTracker[assetPacks[i].groupName][j][k].length + Aux.formatForceIfAttributeArrayForVerbose(matchCountTracker[assetPacks[i].groupName][j][k]), verboseLog_ForceIfList, out_writeVerbose);
            }
        }
    }
    ...
    for (let i = 0; i < assetPacks.length; i++)
    {
        ...
        for (let j = 0; j < currentAssetPack.flattenedSubgroups.length; j++)
        {
            ...
            {
                Aux.logVerbose("Replacing " + assetPacks[i].groupName + " at position " + j + " (" + assetPacks[i].subgroups[j].id + ") with the subgroups best-matched by forceIf attributes.", verboseLog_ForceIfList, out_writeVerbose);
            }
        }
    }
...
function filterSubgroupsByConsistency(assetPacks, consistencyInfo, NPCinfo, pre_consistency, verboseLog_Consistency, out_writeVerbose, logMessage)
{
    ...
    if (assetPacks.length === 0)
    {
        ...
        Aux.logVerbose("None of the available config files match the consistency asset pack. Generating random permutation for this NPC.", verboseLog_Consistency, out_writeVerbose);
