...
module.exports = function()
{
    ...
    { 
        ...
        if (settings.bVerboseMode_Assets_Failed === true && verboseLogController.reportThisNPC === true)
        {
...
function chooseRandomPermutation(assetPackSettings, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, NPCinfo, verboseReport, raceAliases, verboseLogController, xelib, logMessage, attributeCache)
{
    ...
    {
       permutationToReturn = generatePermutation(assetPackSettings, NPCrecordHandle, userForcedAssignment, NPCinfo, consistencyRecords[NPCinfo.consistencyIndex], previousIterationOutput, removedSubgroupTracker, bEnableBodyGenIntegration, currentVerboseReport, verboseLogController, xelib, logMessage, attributeCache);
       ...
       else
       {
        ...
        if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false && Object.keys(BGcategorizedMorphs[NPCinfo.gender]).length > 0) // gender check to avoid wasting time on NPCs that don't have morphs assigned
        {
            ...
            // if no morph could be generated with the current permutation's restraints, look to see if a morph would be available to this NPC without the constraints imposed by this permutation
            if (chosenMorph === undefined)
            {
                Aux.logVerbose("No morph could be selected in conjunction with the current permutation. Checking if a morph would be valid without the restriction imposed by this permutation.", verboseReport, verboseLogController);
            ...
            // OR if the consistency morph was INVALID due to the user changing its distribution rules or modding the NPC's attributes (indicated by passConsistencyMessage.message being not undefined)
            ...
            // if the consistency morph for this NPC is VALID but the chosen morph is NOT the consistency morph, store both as fallback options
...
function generatePermutation (assetPackSettings, NPCrecordHandle, userForcedAssignment, NPCinfo, consistencyInfo, previousIterationOutput, removedSubgroupTracker, bEnableBodyGenIntegration, verboseReport, verboseLogController, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    let permutation = {};
    ...
    if (Object.keys(previousIterationOutput).length === 0)
    {
        ...
        bFilteredByUserForceList = filterSubgroupsByForceList(assetPackSettings, userForcedAssignment, verboseLog_UserForceList, verboseLogController, logMessage);
    ...
    while (bPermutationCreated === false)
    {
        ...
        {
            Aux.logVerbose("Choosing a seed subgroup from the following list:\n" + Aux.formatAssetPacksForVerbose(assetPackSettings, true), verboseReport, verboseLogController);
            subgroupChoiceListPrimary = generateSubgroupChoiceList(assetPackSettings, verboseReport, verboseLogController);

            if (bFirstTimeFunctionCalled === false && filterSubgroupsByPreviousPermutation(removedSubgroupTracker, assetPackSettings, previousIterationOutput, subgroupChoiceListPrimary, verboseReport, verboseLogController) === false)
            {
                break;
            }
            ...
            if (subgroupChoiceListPrimary.length === 0) // if no valid seed subgroups remain, try removing previously applied filters
            {
                ...
                {
                    assetPackSettings = angular.copy(pre_consistency);
        ...
        if (filterRequiredSubgroups(chosenAssetPack.flattenedSubgroups, permutation.requiredSubgroups, 0, verboseLogController, verboseReport) === false)
        {
            ...
            while (bValidChoice === false && flattenedSubgroupsSecondary[i].length > 0)
            {
                ...
                {                    
                    ...
                    if (filterRequiredSubgroups(trialFlattenedSubgroups, trialPermutation.requiredSubgroups, i, verboseLogController, verboseReport) !== false && bSubgroupRequirementsMet(trialSubgroup, trialPermutation, verboseLogController, verboseReport) === true) // returns false if none of the remaining subgroups are compatible with the filter
                    {
                        ...
                        if (filterExcludedSubgroups(trialFlattenedSubgroups, trialPermutation.excludedSubgroups, i, verboseLogController, verboseReport) !== false)
                        {
                            bValidChoice = true;
                        }
                        else
...
function filterSubgroupsByForceList(assetPacks, userForcedAssignment, verboseLog_UserForceList, verboseLogController, logMessage)
{
    bFiltered = false;
...
function generateSubgroupChoiceList(assetPackSettings, verboseReport, verboseLogController)
{
    let validAssetPacks = [];
    let chosenAssetPack = {};
    let allSubgroups = [];
...
function filterExcludedSubgroups(flattenedSubgroups, exclusions, startIndex, verboseLogController, verboseReport)
{
    if (exclusions.length > 0)
    {
        Aux.logVerbose("Filtering subgroups based on the following exclusions: \n" + exclusions.join(", "), verboseReport, verboseLogController);
    }
    ...
    for (let i = startIndex; i < flattenedSubgroups.length; i++)
    {
        ...
        {
            Aux.logVerbose("No subgroups at index " + i + " are compatible with the excluded subgroups for this position. Filtering by excluded subgroups has failed.", verboseReport, verboseLogController);
            return false;
        }
    }
...
function filterSubgroupsByPreviousPermutation(tracker, assetPackSettings, previousIterationOutput, subgroupChoiceListPrimary, verboseReport, verboseLogController)
{
    ...
    {
        Aux.logVerbose("\tAll possible seed subgroups from the current set of available subgroups have been tried. Attempting to remove a different subgroup from the available list.", verboseReport, verboseLogController);
        while (bSugroupHasAlreadyBeenRemoved(tracker, verboseReport, verboseLogController) === true)
        {
            tracker.chosenUniqueSeeds = [];
            ...
            if (tracker.splicedSubgroupCoordinates.length > 0)
            {
                ...
                Aux.logVerbose("\tRestoring previously removed subgroup " + tracker.splicedSubgroup.id + " to " + tracker.packToTrim.groupName, verboseReport, verboseLogController, verboseReport, verboseLogController);
            }
            ...
            if (tracker.currentSubgroupIndex === previousIterationOutput.generatedPermutationSubgroups[tracker.currentPermutationIndex].length) 
            {
                ...
                Aux.logVerbose("\tNo subgroups remain available to remove in asset pack " + tracker.packToTrim.groupName + ". Moving to the next asset pack.", verboseReport, verboseLogController);
            }
            ...
            {
                Aux.logVerbose("\tNo asset packs from which subgroups can be subtracted remain. No more unique permutations can be generated.", verboseReport, verboseLogController);
            ...
            {
                Aux.logVerbose("\tCannot remove subgroup " + tracker.subgroupIDtoTrim + " from " + tracker.packToTrim.groupName + " because it is the only available subgroup at that position.", verboseReport, verboseLogController);
                tracker.currentSubgroupIndex++;
                continue;
            }
            else
            {
                Aux.logVerbose("\tRemoving subgroup " + tracker.subgroupIDtoTrim + " from " + tracker.packToTrim.groupName, verboseReport, verboseLogController);
    ...
    {
        Aux.logVerbose("\tTrying a different seed subgroup from the current set of available subgroups.", verboseReport, verboseLogController);
    }
    return true;
}

function bSugroupHasAlreadyBeenRemoved(tracker, verboseReport, verboseLogController)
{
    ...
    for (let i = 0; i < tracker.previouslyRemovedSubgroups.length; i++)
    {
        ...
        {
            Aux.logVerbose("This subgroup has already been removed when operating on another previously generated permutation, and all resulting new permutations have failed. Moving on to the next subgroup.", verboseReport, verboseLogController);
            return true;
        }
    }
