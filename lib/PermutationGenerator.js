...
function generatePermutation (assetPackSettings, NPCrecordHandle, userForcedAssignment, NPCinfo, consistencyInfo, previousIterationOutput, removedSubgroupTracker, bEnableBodyGenIntegration, verboseReport, verboseLogController, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    while (bPermutationCreated === false)
    {
        ...
        {
            seedAssetPackList = angular.copy(assetPackSettings); // the seed list should be independent from the secondary list so that if a group gets removed from the seed list it can still be chosen in the secondary list.
            ...
            subgroupChoiceListPrimary = generateSubgroupChoiceList(seedAssetPackList, verboseReport, verboseLogController);

            if (bFirstTimeFunctionCalled === false && filterSubgroupsByPreviousPermutation(removedSubgroupTracker, seedAssetPackList, previousIterationOutput, subgroupChoiceListPrimary, verboseReport, verboseLogController) === false)
            {
                break;
            }
...
function filterSubgroupsByPreviousPermutation(tracker, assetPackSettings, previousIterationOutput, subgroupChoiceListPrimary, verboseReport, verboseLogController)
{
    ...
    if (tracker.numPossibleSeeds === undefined) // first time this function is called
    {
        tracker.numPossibleSeeds = Aux.getArrayUniquesByValue(subgroupChoiceListPrimary).length;
        tracker.chosenUniqueSeeds = [];
    }
    
    if (tracker.chosenUniqueSeeds.length === tracker.numPossibleSeeds) // get rid of the current possibilities
    {
    ...
    // get rid of subgroups in subgroupChoiceListPrimary that have already been tried
    for (let i = 0; i < subgroupChoiceListPrimary.length; i++)
    {
        for (let j = 0; j < tracker.chosenUniqueSeeds.length; j++)
        {
            if (subgroupChoiceListPrimary[i].id === tracker.chosenUniqueSeeds[j][0] && subgroupChoiceListPrimary[i].parentAssetPack === tracker.chosenUniqueSeeds[j][1])
            {
                subgroupChoiceListPrimary.splice(i, 1);
                i--;
                break;
            }
        }
