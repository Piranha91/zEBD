...
module.exports = function()
    {
        ...
        PG.generateAssetPackPermutations = function (assetPackSettings, raceGroupDefinitions, patcherSettings, trimPaths, helpers)
        {
            ...
            for (let i = 0; i < assetPackSettings.length; i++)
            {
                ...
                currentPermutations = generatePermutations(variants, raceGroupDefinitions, patcherSettings.permutationBuildUpLogger, IO.logPermutationBuildup, assetPackSettings[i].subgroups, helpers.logMessage, patcherSettings.bEnableBodyGenIntegration);
        ...
        PG.generateInlinePermutations = function (assetPackSettings, bEnableBodyGenIntegration)
        {
            //let IO = require('./IO.js')(logDir, fh);
            ...
            for (let i = 0; i < assetPackSettings.length; i++)
            {
                currentPermutations = generatePermutations(assetPackSettings[i].flattenedSubgroups, [], false, undefined, assetPackSettings[i].subgroups, bEnableBodyGenIntegration);
                finalizePermutations(currentPermutations, assetPackSettings[i].groupName, assetPackSettings[i].gender);
                Aux.copyArrayInto(currentPermutations, allPermutations);
            }
        ...
        // new in verstion 1.9. Called during permuation assembly
        /*
        PG.bResolveSubgroupPermutationConflicts = function(subgroup, permutation, subgroupHierarchy, bEnableBodyGenIntegration)
        {
            ...
            if (bCompatibilizeExcludedSubgroups(subgroup, permutation, false) === false)  { return false; } // checks only subgroup (removeDuplicates = false to avoid screwing up the reverse check on the following line)
            if (bCompatibilizeExcludedSubgroups(permutation, subgroup, true) === false)  { return false; } // checks only permutation 
        ...
        */
        ...
        PG.appendSubgroupToPH = function(subgroup, permutation)
        {
            ...
            permutation.allowedAttributes = [...convertAttributePairsToObjects(subgroup.allowedAttributes, subgroup), ...permutation.allowedAttributes];
            permutation.disallowedAttributes = [...convertAttributePairsToObjects(subgroup.disallowedAttributes, subgroup), ...permutation.disallowedAttributes];
            permutation.forceIfAttributes = [...convertAttributePairsToObjects(subgroup.forceIfAttributes, subgroup), ...permutation.forceIfAttributes];
...
function convertAttributePairsToObjects(attributePairs, sourceSubgroup)
{
    let Aobj;
    let objectList = [];
    for (let i = 0; i < attributePairs.length; i++)
...
function generatePermutations(variants, restrictionGroupDefs, permutationBuildUpLogger, logPermutationBuildup, subgroupHierarchy, logMessage, bEnableBodyGenIntegration)
{
    ...
    let subPermutations = generatePermutations(otherColumns, restrictionGroupDefs, permutationBuildUpLogger, logPermutationBuildup, subgroupHierarchy, logMessage, bEnableBodyGenIntegration); // recursively call this function to generate all permutation combinations from the columns to the right of this one.
    ...
    {     
        for (let j = 0; j < subPermutations.length; j++)
        {
            ...
            if (checkPermutationRequireAllowExclude(firstColumn[i], subPermutations[j], subgroupHierarchy, currentSubgroupPos) === true) // this is a separate function from bResolveSubgroupPermutationConflicts() to avoid wasting time on unnecessary angular.copy() calls
            {
                ...
                if (bResolveSubgroupPermutationConflicts(subGroupA, permutationB, subgroupHierarchy, bEnableBodyGenIntegration) === true)
                {                 
...
function bResolveSubgroupParentConflicts(subgroup, parent, subgroupHierarchy, bEnableBodyGenIntegration)
{
    ...
    if (bCompatibilizeExcludedSubgroups(subgroup, parent, false) === false)  { return false; } //removeDuplicates = false to avoid screwing up the reverse check on the following line
    if (bCompatibilizeExcludedSubgroups(parent, subgroup, true) === false)  { return false; } 
...
bResolveSubgroupPermutationConflicts = function(subgroup, permutation, subgroupHierarchy, bEnableBodyGenIntegration)
{
    // the following actually affect assignment:
    if (bCompatibilizeRequiredSubgroups(subgroup, permutation, subgroupHierarchy) === false) { return false; }
    if (bCompatibilizeExcludedSubgroups(subgroup, permutation, false) === false)  { return false; } // checks only subgroup (removeDuplicates = false to avoid screwing up the reverse check on the following line)
...
function bCompatibilizeRequiredSubgroups(mutate1, mutate2, subgroupHierarchy)
{
    ...
    if (bCompatibilizeRequiredSubgroupIntersection(mutate1, mutate2, false) === false)  { return false; } // checks only subgroup (removeDuplicates = false to avoid screwing up the reverse check on the following line)
    if (bCompatibilizeRequiredSubgroupIntersection(mutate2, mutate1, true) === false)  { return false; } // checks only permutation
   
    return true;
}

function bCompatibilizeRequiredSubgroupIntersection(mutate, doNotMutate, bRemoveDuplicates)
{
...
function bCompatibilizeExcludedSubgroups(mutate, doNotMutate, bRemoveDuplicates)
{
