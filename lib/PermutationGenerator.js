...
module.exports = function(logDir, fh)
    {
        let PG = {};
        PG.generateAssetPackPermutations = function (patcherSettings, trimPaths, helpers)
        {
            ...
            {
                helpers.logMessage("Generating permutations from " + patcherSettings.assetPackSettings[i].groupName);
                variants = [];
                if (patcherSettings.bVerboseMode === true) { IO.logVerbose("Getting bottom-node subgroups in Pack Settings: " + patcherSettings.assetPackSettings[i].groupName); }
                if (patcherSettings.permutationBuildUpLogger === true) { IO.logPermutationBuildup(undefined, false, patcherSettings.assetPackSettings[i].groupName); }
                for (let j = 0; j < patcherSettings.assetPackSettings[i].subgroups.length; j++) // generate every possible variant of each top-level subgroup. Each subgroup must be a unique object rather than a shared reference address.
                {
                    generateSubgroupVariants(patcherSettings.assetPackSettings[i].subgroups[j], currentSubgroupVariant, new transferSubgroupInfo(), patcherSettings.raceGroupDefinitions, patcherSettings.bVerboseMode, IO.logVerbose, patcherSettings.assetPackSettings[i].subgroups, patcherSettings.bEnableBodyGenIntegration);
                ...
                if (patcherSettings.logVerbose === true) { IO.logVerbose( "\n" + patcherSettings.assetPackSettings[i].groupName + ": generated permutation arrays for " + variants.length + " top-level subgroups\n"); }

                currentPermutations = generatePermutations(variants, patcherSettings.raceGroupDefinitions, patcherSettings.bVerboseMode, IO.logVerbose, patcherSettings.permutationBuildUpLogger, IO.logPermutationBuildup, patcherSettings.assetPackSettings[i].subgroups, helpers.logMessage, patcherSettings.bEnableBodyGenIntegration);
                finalizePermutations(currentPermutations, patcherSettings.assetPackSettings[i].groupName, patcherSettings.assetPackSettings[i].gender);
...
class permutationHolder
{
    constructor(name)
    {
        this.assetPackSettingsGeneratedFrom = name;
        this.nameString = "";
        ...
        this.addKeywords = [],
        this.paths = [],
        this.allowedBodyGenPresets = [];
        this.disallowedBodyGenPresets = [];
        ...
        this.emptyFlagAB = false; // same but for BodyGen presets
        this.NPCsAssignedTo = []; // for the log; assigned during patch function
        // the following is used by patcherOps
        this.topLevelSubgroupsSatisfied = [];
    }
...
class transferSubgroupInfo
{
    constructor() // this is only ever used to create the empty parent subgroup for the first recursion of generateSubgroupVariants()
    {
        ...
        this.allowedBodyGenPresets = [];
        this.disallowedBodyGenPresets = [];
        this.emptyFlagAB = true; // defaults to true, make sure to set to false if allowedBodyGenPresets gets populated
    }
    ...
    static copyComponents(copyFrom, copyTo)
    {
        ...
        Aux.copyArrayInto(copyFrom.allowedBodyGenPresets, copyTo.allowedBodyGenPresets);
        Aux.copyArrayInto(copyFrom.disallowedBodyGenPresets, copyTo.disallowedBodyGenPresets);

        Aux.getArrayUniques(copyTo.allowedRaces);
        Aux.getArrayUniques(copyTo.disallowedRaces);
        Aux.getArrayUniques(copyTo.allowedBodyGenPresets);
        Aux.getArrayUniques(copyTo.disallowedBodyGenPresets);
    }
}
...
function generateSubgroupVariants(subgroup, permArray, parentSubgroup, restrictionGroupDefs, bVerboseMode, verboseLogger, subgroupHierarchy, bEnableBodyGenIntegration)
{
    ...
    if (sgClone.allowedBodyGenPresets.length === 0)
    {
        sgClone.emptyFlagAB = true; // see above
    }
    else
    {
        sgClone.emptyFlagAB = false;
    }
...
function generatePermutations(variants, restrictionGroupDefs, bVerboseMode, verboseLogger, permutationBuildUpLogger, logPermutationBuildup, subgroupHierarchy, logMessage, bEnableBodyGenIntegration)
{
    ...
    if (variants.length === 1) // if in the bottom layer, simply return the last array column
    {
        for (let i = 0; i < variants[0].length; i++)
        {
            ...
            if (variants[0][i].emptyFlagAB === true)
            {
                PH.emptyFlagAB = true;
            }
...
function finalizePermutations(permutations, assetPackSettingsGeneratedFrom, gender)
{
    for (let i = 0; i < permutations.length; i++)
    {
        permutations[i].assetPackSettingsGeneratedFrom = assetPackSettingsGeneratedFrom;
...
function bResolveSubgroupParentConflicts(subgroup, parent, subgroupHierarchy, bVerboseMode, verboseLogger, bEnableBodyGenIntegration)
{
    ...
    {
        if (bCompatibilizeAllowedBodyGen(subgroup, parent, bVerboseMode, verboseLogger, false) === false) { return false; } // duplicates handled in copy function to avoid logic errors
        if (bCompatibilizeDisallowedBodyGen(subgroup, parent, bVerboseMode, verboseLogger, false) === false) { return false; } // duplicates handled in copy function to avoid logic errors
    }
...
function bResolveSubgroupPermutationConflicts(subgroup, permutation, subgroupHierarchy, bVerboseMode, verboseLogger, bEnableBodyGenIntegration)
{
    ...
    if (bEnableBodyGenIntegration === true)
    {
        if (bCompatibilizeAllowedBodyGen(subgroup, permutation, bVerboseMode, verboseLogger, false) === false) { return false; } // trims only subgroup
        if (bCompatibilizeAllowedBodyGen(permutation, subgroup, bVerboseMode, verboseLogger, false) === false) { return false; } // trims only permutation     
        ...
        if (bCompatibilizeDisallowedBodyGen(permutation, subgroup, bVerboseMode, verboseLogger, false) === false) { return false; } // checks only permutation
    }
...
function compatibilizeAllowedAttributes(mutate, doNoteMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.allowedAttributes, doNoteMutate.allowedAttributes)};
}

function compatibilizeDisallowedAttributes(mutate, doNoteMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.disallowedAttributes, doNoteMutate.disallowedAttributes)};
}

function compatibilizeForceIfAttributes(mutate, doNoteMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.forceIfAttributes, doNoteMutate.forceIfAttributes)};
}
...
function bCompatibilizeAllowedBodyGen(mutate, doNotMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    // These checks should not be necessary but leaving in place currently
    if (mutate.allowedBodyGenPresets.length === 0 && mutate.emptyFlagAB === false)
    {
        return false;
    }
    else if (doNotMutate.allowedBodyGenPresets.length === 0 && doNotMutate.emptyFlagAB === false)
    {
        return false;
    }
    // END

    // if both allowedBodyGenPresets lists are empty, simply return true
    else if (mutate.emptyFlagAB === true && doNotMutate.emptyFlagAB === true)
    {
        return true;
    }
    // if mutate has allowedBodyGenPresets and doNotMutate doesn't, simply return true
    else if (mutate.emptyFlagAB === false && doNotMutate.emptyFlagAB === true)
    {
        return true;
    }
    // if mutate has no allowedBodyGenPresets but doNotMutate does, copy its allowedBodyGenPresets into mutate and disable the empty flag
    else if (mutate.emptyFlagAB === true && doNotMutate.emptyFlagAB === false)
    {
        mutate.allowedBodyGenPresets = doNotMutate.allowedBodyGenPresets.slice(); // shallow copy
        mutate.emptyFlagAB = false;
    }
    // if both mutate and doNotMutate have allowedBodyGenPresets
    else
    {     
        Aux.getArrayIntersectionWithTarget(mutate.allowedBodyGenPresets, doNotMutate.allowedBodyGenPresets, bVerboseMode, verboseLogger)
        // if mutate no longer does after computing intersection with doNotMutate, then the two are invalid together
        if (mutate.allowedBodyGenPresets.length === 0)
        {
            return false;
        }     
    }
    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.allowedBodyGenPresets, doNotMutate.allowedBodyGenPresets); }
    return true;
}
...
function bCompatibilizeDisallowedBodyGen(mutate, doNotMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    ...
    // If any from parent are missing in current, add them to current
    Aux.addMissingArrayElements(doNotMutate.disallowedBodyGenPresets, mutate.disallowedBodyGenPresets);

    // If any disallowedBodyGenPresets are present in allowedBodyGenPresets, remove them
    for (let i = 0; i < mutate.allowedBodyGenPresets.length; i++)
    {
        if (doNotMutate.disallowedBodyGenPresets.includes(mutate.allowedBodyGenPresets[i]))
        {
            mutate.allowedBodyGenPresets.splice(i, 1);
            i--;
        }
    }
    // if current subgroup previously had allowedBodyGenPresets but no longer does after comparison with parent's disallowed races, they are incompatible
    if (mutate.allowedBodyGenPresets.length === 0 && mutate.emptyFlagAB === false)
    {
        return false;
    }

    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.disallowedBodyGenPresets, doNotMutate.disallowedBodyGenPresets);}
