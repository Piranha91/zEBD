...
function bCompatibilizeDisallowedBodyGen(mutate, doNotMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    ...
    if (pruneDisallowedBodyGenFromAllowed(mutate.allowedBodyGenDescriptors, doNotMutate.disallowedBodyGenDescriptors) === false)
    {
        return false;
    }
...
function pruneDisallowedBodyGenFromAllowed(mutate, exclusions)
{
    for (let category in mutate.allowedBodyGenDescriptors)
    {
        for (let i = 0; i < exclusions.length; i++)
        {
            if (category === exclusions[i].category)
            {
                for (let j = 0; j < mutate.allowedBodyGenDescriptors[category].length; j++)
                {
                    if (mutate.allowedBodyGenDescriptors[category][j] === exclusions[i].value)
                    {
                        mutate.allowedBodyGenDescriptors[category].splice(j, 1);
                        j--;
                    }
                }
            }
