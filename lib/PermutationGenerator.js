...
function compatibilizeWeightRange(mutate, doNotMutate, bVerboseMode, verboseLogger)
{
    ...
    for (let i = 0; i < 2; i++)
    {    
        if (Number.isNaN(mutate.weightRange[i]) && !Number.isNaN(doNotMutate.weightRange[i]))
        {
            mutate.weightRange[i] = doNotMutate.weightRange[i];
        }
    }

    if (!Number.isNaN(mutate.weightRange[0]) && !Number.isNaN(doNotMutate.weightRange[0]))
    {
        mutate.weightRange[0] = Math.max(mutate.weightRange[0], doNotMutate.weightRange[0]);
    }

    if (!Number.isNaN(mutate.weightRange[1]) && !Number.isNaN(doNotMutate.weightRange[1]))
    {
