...
class permutationHolder
{
    constructor(name)
    {
        ...
        this.addKeywords = [],
        this.paths = [],
        this.allowedBodyGenDescriptors = [];
...
class transferSubgroupInfo
{
    constructor() // this is only ever used to create the empty parent subgroup for the first recursion of generateSubgroupVariants()
    {
        ...
        this.allowedBodyGenDescriptors = [];
        this.disallowedBodyGenDescriptors = [];
        this.emptyFlagAB = true; // defaults to true, make sure to set to false if allowedBodyGenDescriptors gets populated
    }
    ...
    static copyComponents(copyFrom, copyTo)
    {
        ...
        Aux.copyArrayInto(copyFrom.allowedBodyGenDescriptors, copyTo.allowedBodyGenDescriptors);
        ...
        Aux.getArrayUniques(copyTo.allowedBodyGenDescriptors);
...
{
    // These checks should not be necessary but leaving in place currently
    if (mutate.allowedBodyGenDescriptors.length === 0 && mutate.emptyFlagAB === false)
    {
        return false;
    }
    else if (doNotMutate.allowedBodyGenDescriptors.length === 0 && doNotMutate.emptyFlagAB === false)
    {
        return false;
    }
    // END

    // if both allowedBodyGenDescriptors lists are empty, simply return true
    else if (mutate.emptyFlagAB === true && doNotMutate.emptyFlagAB === true)
    {
        return true;
    }
    // if mutate has allowedBodyGenDescriptors and doNotMutate doesn't, simply return true
    else if (mutate.emptyFlagAB === false && doNotMutate.emptyFlagAB === true)
    {
        return true;
    }
    // if mutate has no allowedBodyGenDescriptors but doNotMutate does, copy its allowedBodyGenDescriptors into mutate and disable the empty flag
    else if (mutate.emptyFlagAB === true && doNotMutate.emptyFlagAB === false)
    {
        mutate.allowedBodyGenDescriptors = doNotMutate.allowedBodyGenDescriptors.slice(); // shallow copy
        mutate.emptyFlagAB = false;
    }
    // if both mutate and doNotMutate have allowedBodyGenDescriptors
    else
    {     
        Aux.getArrayIntersectionWithTarget(mutate.allowedBodyGenDescriptors, doNotMutate.allowedBodyGenDescriptors, bVerboseMode, verboseLogger)
        // if mutate no longer does after computing intersection with doNotMutate, then the two are invalid together
        if (mutate.allowedBodyGenDescriptors.length === 0)
        {
            return false;
        }     
    }
    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.allowedBodyGenDescriptors, doNotMutate.allowedBodyGenDescriptors); }
    return true;
}
...
function bCompatibilizeDisallowedBodyGen(mutate, doNotMutate, bVerboseMode, verboseLogger, bRemoveDuplicates)
{
    ...
    // If any disallowedBodyGenDescriptors are present in allowedBodyGenDescriptors, remove them
    for (let i = 0; i < mutate.allowedBodyGenDescriptors.length; i++)
    {
        if (doNotMutate.disallowedBodyGenDescriptors.includes(mutate.allowedBodyGenDescriptors[i]))
        {
            mutate.allowedBodyGenDescriptors.splice(i, 1);
            i--;
        }
    }
    // if current subgroup previously had allowedBodyGenDescriptors but no longer does after comparison with parent's disallowed races, they are incompatible
    if (mutate.allowedBodyGenDescriptors.length === 0 && mutate.emptyFlagAB === false)
    {
        return false;
    }

    if (bRemoveDuplicates === true) { Aux.removeArrayDuplicates(mutate.disallowedBodyGenDescriptors, doNotMutate.disallowedBodyGenDescriptors);}
