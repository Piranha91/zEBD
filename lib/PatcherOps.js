...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.choosePermutation_BodyGen = function(NPCrecordHandle, NPCinfo, permutationsByRaceGender, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, consistencyRecords, bEnableConsistency, bEnableBodyGenIntegration, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCpermutations, linkedNPCbodygen, NPClinkGroup, attributeCache, logMessage)
    {
        ...
        if (bSkipSelection === false)
        {
            ...
            // CONSISTENCY:
            permutationToReturn = drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, xelib, logMessage, attributeCache);
            if (permutationToReturn != undefined) 
            { 
        ...
        {
            logMessage("\nAsset Assignment: no permutations satisfied criteria for " + NPCinfo.name + " (" + NPCinfo.formID + "). Skipping this NPC.");
...
function chooseRandomPermutation(permutations, choiceArray, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyAssignments, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    let validPermutations = []; // cache of valid permutations (for storing permutations that themselves conform to NPC's requirements, but do not pair with any BodyGen morphs)
    let chosenMorph;
    ...
    while (bValidChoice === false && choiceArray.length > 0)
    {
        ...
        {
            validPermutations.push(permutationToReturn);
            chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyAssignments, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
            
            if (chosenMorph === undefined) // if the selected permutation doesn't match with a bodygen, call it invalid and pick again.
            {
                bValidChoice = false;
            }
            else
    ...
    if (bValidChoice === false && validPermutations.length > 0) // if there are one or more permutations that conform to requirements but don't have an allowed BodyGen, pick the first one that worked and then pick a random BodyGen.
    {
        permutationToReturn = validPermutations[0];
        logMessage("\nNPC " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") was not compatible with a permutation + bodygen morph combination due to mutually conflicting restrictions. A valid permutation was assigned, but BodyGen assignment will proceed without respecting this permutation's restrictions.")
    }
...
function bPermutationValidForCurrentNPC(NPCrecordHandle, permutation, userForcedSubgroups, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    for (let i = 0; i < permutation.subgroups.length; i++) // loop through all contributing subgroups
    {
        ...
        {
            updateFailureModes(failureModes, "Distribution disabled for non-user-forced NPCs");
            return false;
        }
        ...
        {
            updateFailureModes(failureModes, "Distribution disallowed for unique NPCs");
            return false;
        }
        ...
        {
            updateFailureModes(failureModes, "Distribution disallowed for non-unique NPCs");
            return false;
        }
        ...
        for (let j = 0; j < permutation.subgroups[i].disallowedAttributes.length; j++) 
        {
            if (Aux.bAttributeMatched(permutation.subgroups[i].disallowedAttributes[j][0], permutation.subgroups[i].disallowedAttributes[j][1], NPCrecordHandle, logMessage, xelib, attributeCache)) // if current NPC matches current subgroup's disallowed attribute
            {
                ...
                {
                    updateFailureModes(failureModes, "NPC has disallowed attribute: (" + permutation.subgroups[i].disallowedAttributes[j][0] + ": " + permutation.subgroups[i].disallowedAttributes[j][1] + " (imposed by subgroup " + permutation.subgroups[i].id + ")");
                    return false;
                }
        ...
        if ((permutation.subgroups[i].allowedAttributes.length > 0 && bAttributeMatched === false) && userForcedSubgroups.includes(permutation.subgroups[i].name) === false && permutation.forceIfSubgroupsApplied.includes(permutation.subgroups[i].name) === false) // if the NPC lacks at least one of this subgroup's allowed attributes AND the subgroup is not forced by either the user or a forceIF, disallow it
        {
            ...
            updateFailureModes(failureModes, tmpFailureStr);
            return false;
        }
    }
    return true;
...
function updateFailureModes(failureModes, failureModeString)
{
...
function drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, xelib, logMessage, attributeCache)
{
    let failureModes = {};
    if (bEnableConsistency === true && bNPCfoundInConsistencyRecords === true)
    {
        ...
        {
            logMessage("\nAsset Assignment: the permutation specified in the consistency file for " + NPCinfo.name + " (" + NPCinfo.formID + ") could not be assigned consistently with the subgroups defined in ForceNPCList.json.");
        }
        else
        {
            logMessage("\nAsset Assignment: the permutation specified in the consistency file for " + NPCinfo.name + " (" + NPCinfo.formID + ") could not be assigned. Assigning a random permutation.");
...
function filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bAssetsForcedByUser, logMessage, NPCinfo, bAllowOtherRGCombos) // bAllowOtherRGCombos significantly increases processing time
{
    ...
    if (bAssetsForcedByUser === true)
    {
        ...
        {
            logMessage("\nAsset Assignment: no generated permutations comply with the subgroups forced in ForceNPCList.json for " + NPCinfo.name + " (" + NPCinfo.formID + "). Assigning a random permutation.");
        }
        else
        {
