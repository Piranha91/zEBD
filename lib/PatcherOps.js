    ...
    while (bValidChoice === false && choiceArray.length > 0)
    {
        ...
        if (bValidChoice === true)
        {
            if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false)
            {
                ...
                chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage, passConsistencyMessage);
                ...
                {
                    chosenMorph2 = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, undefined, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage, {});
...
function bPermutationValidForCurrentNPC(NPCrecordHandle, permutation, userForcedSubgroups, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    {
        if (userForcedSubgroups.includes(permutation.subgroups[i].id) === false) // if the user has forced the current subgroup then it is automatically valid so skip the following checks
        {
            // "Distribution enabled"
            if (permutation.subgroups[i].distributionEnabled === false)
            {
            ...
            // "Allow unique NPCs"
            if (permutation.subgroups[i].allowUnique === false && NPCinfo.isUnique === true)
            {
            ...
            if (permutation.subgroups[i].allowNonUnique === false && NPCinfo.isUnique === false)
            {
            ...
            // "weight range"
            if (Aux.isValidNumber(permutation.subgroups[i].weightRange[0]) && NPCinfo.weight < permutation.subgroups[i].weightRange[0])
            {
                Aux.updateFailureModes(failureModes, "NPC weight (" + NPCinfo.weight.toString() + ") <  " + permutation.subgroups[i].weightRange[0].toString());
                return false;
            }
            if (Aux.isValidNumber(permutation.subgroups[i].weightRange[1]) && NPCinfo.weight > permutation.subgroups[i].weightRange[1])
            {
                Aux.updateFailureModes(failureModes, "NPC weight (" + NPCinfo.weight.toString() + ") >  " + permutation.subgroups[i].weightRange[1].toString());
                return false;
            }

            bSubgroupForced = permutation.forceIfSubgroupsApplied.includes(permutation.subgroups[i].id); // true if subgroup is forced via ForceIF attributes
            ...
            bMissingAllAllowedAttriubutes = (permutation.subgroups[i].allowedAttributes.length > 0 && bAttributeMatched === false);
...
function drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, xelib, logMessage, attributeCache)
{
    ...
    if (bEnableConsistency === true && bNPCfoundInConsistencyRecords === true)
    {
        ...
        else if (bPermutationValidForCurrentNPC(NPCrecordHandle, permutationToReturn, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) === true)
        {
            ...
            if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false)
            {
                let chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage, passConsistencyMessage);
