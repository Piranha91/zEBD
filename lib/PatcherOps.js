...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.getNPCinfo = function(NPCrecordHandle, consistencyRecords, xelib)
    {
        ...
        NPCinfo.masterRecordFile = xelib.GetFileName(xelib.GetElementFile(xelib.GetMasterRecord(NPCrecordHandle)));
    ...
    PO.getUserForcedAssignment = function(NPCinfo, forcedNPCAssignments)
    {
        let NPCForcedSubgroupsIndex = findNPCForcedIndex(forcedNPCAssignments, NPCinfo);// for user-assigned subgroup IDs
        return forcedNPCAssignments[NPCForcedSubgroupsIndex];
    }

    PO.assignNPCheight = function(NPCrecordHandle, NPCinfo, bEnableConsistency, consistencyRecords, heightSettings)
    {
        let distMode;
        let lowerBound;
        let upperBound;
        ...
        // consistency
        if (bEnableConsistency === true && NPCinfo.consistencyIndex >= 0 && consistencyRecords[NPCinfo.consistencyIndex].height !== undefined)
        {
            bGenerateRandom = false;
            height = consistencyRecords[NPCinfo.consistencyIndex].height;

            // validate to make sure settings haven't changed since last run
            switch(NPCinfo.gender)
            {
                case "male":
                    if (height < 1 - currentHeightSettings.heightMaleRange || height > 1 + currentHeightSettings.heightMaleRange)
                    {
                        bGenerateRandom = true;
                    }
                    break;
                case "female":
                    if (height < 1 - currentHeightSettings.heightFemaleRange || height > 1 + currentHeightSettings.heightFemaleRange)
                    {
                        bGenerateRandom = true;
                    }
                    break;
        ...
        {
            distMode = currentHeightSettings.distMode;
            switch(NPCinfo.gender)
            {
                case "male": range = parseFloat(currentHeightSettings.heightMaleRange); break;
                case "female": range = parseFloat(currentHeightSettings.heightFemaleRange); break;
            }
            lowerBound = 1 - range;
            upperBound = 1 + range;
        ...
        if (bEnableConsistency === true)
        {
            ...
            updateHeightConsistencyRecord(consistencyRecords[NPCinfo.consistencyIndex], height);
        }
    ...
    PO.patchRaceHeight = function(raceRecordHandle, raceEDID, heightConfiguration)
    {
        for (let i = 0; i < heightConfiguration.length; i++)
        {
            if (heightConfiguration[i].EDID === raceEDID)
            {
				...
				xelib.SetValue(raceRecordHandle, "DATA\\Female Height", heightConfiguration[i].heightMale);
            }
        }
    }
    ...
    PO.choosePermutation = function(NPCrecordHandle, NPCinfo, permutationsByRaceGender, consistencyRecords, bEnableConsistency, userForcedAssignment, bLinkNPCsWithSameName, RGtracker, LinkedNPCNameExclusions, attributeCache, logMessage)
    {
        let allowOtherRGCombos = false; // keep false for now until making option in GUI. True significantly increases computation time. Variable allows forceIfAttributes and forcedSubgroups to look into permutations restricted to NPCs of a race and gender different from that of current NPC

        let bSpecificSubgroupsForcedByUser = false;     // for matching specific user-assigned subgroup IDs to NPCs
        ...
        // get initial permutation list from NPC's race and gender
        ...
        // if NPC assignments are linked by name, check if the current NPC has already been matched and return its permutation if so
        if (bLinkNPCsWithSameName === true && NPCinfo.isUnique === true)
        {
            permutationToReturn = findPermutationInRGTracker(NPCinfo, RGtracker)
            if (permutationToReturn != undefined)
            {
                addAssignedNPCtoPermutation(originalpermutations, permutationToReturn, NPCinfo);
        ...
        {
            bSpecificSubgroupsForcedByUser = true;
        }
        
        // Subgroups forced by user in ForceNPCList.json
        bUserForcedSubgroupsApplied = filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bSpecificSubgroupsForcedByUser, logMessage, NPCinfo, allowOtherRGCombos);
        
        // ForceIfAttributes - prune out permutations that aren't forced for this NPC, if the NPC has the given attribute
        bForceIfAttributesApplied = filterPermutationsbyForceIfAttributes(NPCrecordHandle, bUserForcedSubgroupsApplied, NPCinfo.isUnique, permutations, logMessage, xelib, attributeCache);
        ...
        { 
            addAssignedNPCtoPermutation(originalpermutations, permutationToReturn, NPCinfo);           
            updateRGtracker(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, LinkedNPCNameExclusions, RGtracker);
            return permutationToReturn; 
        }
        ...
        permutationToReturn = chooseRandomPermutation(permutations, choiceArray, NPCrecordHandle, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, NPCinfo.isUnique, xelib, logMessage, attributeCache);

        if (choiceArray.length === 0 && permutationToReturn === undefined)
        {
            logMessage("No permutations satisfied criteria for " + NPCinfo.name + " (" + NPCinfo.formID + "). Skipping this NPC.");
        ...
        else 
        {
            ...
            updateRGtracker(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, LinkedNPCNameExclusions, RGtracker) // store fir linking same-named NPCs

            if (bEnableConsistency === true)
            {
                if (NPCinfo.consistencyIndex === -1) // if no record of this NPC exists in the consistency file, create one
                {
                    createConsistencyRecord(NPCrecordHandle, NPCinfo, consistencyRecords, xelib) // NPCinfo.consistencyIndex gets updated here
                } 
    ...
    PO.generatePatchableRaceList = function(permutations, raceGroupDefinitions)
    {
        let uniques = [];
        for (let i = 0; i < permutations.length; i++)
        {
            for (let j = 0; j < permutations[i].allowedRaces.length; j++)
            {
                uniques.push(permutations[i].allowedRaces[j]);
            }

            if (i % 100 === 0) // the list of allowed races will get huge if the redundant ones aren't dumped periodically. This seems like a reasonable interval.
            {
                uniques = Aux.getArrayUniques(uniques);
            }
        }

        uniques = Aux.getArrayUniques(uniques); // final cleaning for those since the previous clean within the loop

        // if the user has no allowedRaces restrictions at all (unlikely but possible), generate the patchable race list from the group definitions settings
        if (uniques.length === 0)
        {
            uniques = PO.generatePatchableRaceListFallBack(raceGroupDefinitions);
        }

        return uniques;
    };

    PO.generatePatchableRaceListFallBack = function(raceGroupDefinitions)
    {
        let uniques = [];
        for (let i = 0; i < raceGroupDefinitions.length; i++)
        {
            for (let j = 0; j < raceGroupDefinitions[i].entries.length; j++)
            {
                uniques.push(raceGroupDefinitions[i].entries[j]);
            }
        }
    ...
    PO.bNPCisBlocked = function(BlockedNPCs, NPCinfo)
    {
        let blockListIndex = findNPCForcedIndex(BlockedNPCs, NPCinfo); // block list is same format as ForcedList, so using same search function
...
function chooseRandomPermutation(permutations, choiceArray, NPCrecordHandle, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, bNPCisUnique, xelib, logMessage, attributeCache)
{
    ...
    while (bValidChoice === false && choiceArray.length > 0)
    {
        ...
        bValidChoice = bPermutationValidForCurrentNPC(NPCrecordHandle, permutationToReturn, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, bNPCisUnique, xelib, logMessage, attributeCache);
...
function bPermutationValidForCurrentNPC(NPCrecordHandle, permutation, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, bNPCisUnique, xelib, logMessage, attributeCache)
{
    let bValid = true;
    ...
    if (bNPCisUnique === true && permutation.allowUnique === false && bUserForcedSubgroupsApplied === false) // user-forced subgroups override the settings to patch unique NPCs
    {
        return false;
    }

    if (bNPCisUnique === false && permutation.allowNonUnique === false && bUserForcedSubgroupsApplied === false)
    {
        return false;
    }
...
function findNPCForcedIndex(forcedNPCAssignments, NPCinfo)
{
    let index = -1;
    let bHasFormID = false;
    let bHasEDID = false;
    let bHasName = false;

    if (forcedNPCAssignments === undefined || forcedNPCAssignments.length === 0) { return  -1; }
    
    // priority: formID, EDID, name, MasterRecordFile
    for (let i = 0; i < forcedNPCAssignments.length; i++)
    {
        bHasFormID = false;
        bHasEDID = false;
        bHasName = false;
        bHasMasterRecord = false;
        
        if (forcedNPCAssignments[i].rootPlugin !== undefined) { bHasMasterRecord = true; }
        if (forcedNPCAssignments[i].formID !== undefined) { bHasFormID = true; }
        if (forcedNPCAssignments[i].EDID !== undefined) { bHasEDID = true; }
        if (forcedNPCAssignments[i].name !== undefined) { bHasName = true; }
        
        if (bHasFormID === true) // if the user provided a formID, use only that as the identifier
        {
            if (NPCinfo.formID === forcedNPCAssignments[i].formID)
            {
                index = i;
                break;
            }
        ...
        else // if the has not provided formID, match name and EDID if user provides, or just one if not. 
        {
            if (bHasEDID === true && bHasName === true)
            {
                if (NPCinfo.EDID === forcedNPCAssignments[i].EDID && NPCinfo.name === forcedNPCAssignments[i].name)
                {
                    index = i;
                    break;
                }
            } else if (bHasEDID === true && NPCinfo.EDID === forcedNPCAssignments[i].EDID)
            {
                index = i;
                break;
            } else if (bHasName === true && NPCinfo.name === forcedNPCAssignments[i].name)
            {
                index = i;
                break;
            }
...
function drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, xelib, logMessage, attributeCache)
{
    if (bEnableConsistency === true && bNPCfoundInConsistencyRecords === true)
    {
        ...
        if (permutationToReturn !== undefined && bPermutationValidForCurrentNPC(NPCrecordHandle, permutationToReturn, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, NPCinfo.isUnique, xelib, logMessage, attributeCache) === true)
        {
            return permutationToReturn;
        } 
        ...
        {
            logMessage("The permutation specified in the consistency file for " + NPCinfo.name + " (" + NPCinfo.formID + ") could not be assigned consistenly with the subgroups defined in ForceNPCList.json. Assigning a random permutation.");
        }
        else
        {
            logMessage("The permutation specified in the consistency file for " + NPCinfo.name + " (" + NPCinfo.formID + ") could not be assigned. Assigning a random permutation.");
        }
    }
...
function createConsistencyRecord(NPCrecordHandle, NPCinfo, consistencyRecords, xelib)
{
    assignmentRecord = {};
...
function filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bSpecificSubgroupsForcedByUser, logMessage, NPCinfo, allowOtherRGCombos) // allowOtherRGCombos significantly increases processing time
{
    let bUserForcedAttributesApplied = false;
    let userForcedPermutations = [];
    let otherRGPermutations = [];

    if (bSpecificSubgroupsForcedByUser === true)
    {
        for (let i = 0; i < permutations.length; i++)
        {
        ...
        if (allowOtherRGCombos === true && userForcedPermutations.length === 0)
        {
            for (let i = 0; i < permutationsByRaceGender.length; i++)
            {
                ...
                {
                    // first check for the asset pack
                    if (otherRGPermutations[i].assetPackSettingsGeneratedFrom !== userForcedAssignment.forcedAssetPack)
                    {
                        continue;
                    }
                    else if (userForcedAssignment.forcedSubgroups.length === 0) // if the user didn't specify subgroups and only specified the asset pack, add all subgroups from that asset pack
                    {
                        userForcedPermutations.push(otherRGPermutations[i]);
                    }
                    else if (permutationAllowedByUserForceList(userForcedAssignment.forcedSubgroups, otherRGPermutations[j]) === true)
                    {
        ...
        {
            logMessage("No generated permutations comply with the subgroups forced in ForceNPCList.json for " + NPCinfo.name + " (" + NPCinfo.formID + "). Assigning a random permutation.");
        }
        else
        {
...
function filterPermutationsbyForceIfAttributes(NPCrecordHandle, bUserForcedSubgroupsApplied, bNPCisUnique, permutations, logMessage, xelib, attributeCache)
{
    ...
    for (let i = 0; i < permutations.length; i++)
    {
        ...
        if (bpermutationIsForcedForThisNPC === true && bPermutationValidForCurrentNPC(NPCrecordHandle, permutations[i], bUserForcedSubgroupsApplied, true, bNPCisUnique, xelib, logMessage, attributeCache) === true)
        {
...
function permutationAllowedByUserForceList(forcedSubgroups, permutation)
{
    ...
    {
        //if (permutation.contributingSubgroupIDs.includes(forcedSubgroups[i].forcedSubgroupID) === false)
        if (bPermutationContributingSubgroupIDsContain(permutation, forcedSubgroups[i].forcedSubgroupID) === false)
        {
            return false;
        }
    }
...
function findPermutationInRGTracker(NPCinfo, RGtracker)
{
    if (NPCinfo.name === "" || NPCinfo.name === "Prisoner") // avoid linking presets
    {
        return undefined;
    }

    for (let i = 0; i < RGtracker.length; i++)
    {
        if (RGtracker[i][0] === NPCinfo.name && RGtracker[i][1] === NPCinfo.race && RGtracker[i][2] === NPCinfo.gender)
        {
            return RGtracker[i][3];
        }
    }
    return undefined;
}

function updateRGtracker(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, exclusionList, RGtracker)
{
    let bExcluded = false;
    if (bLinkNPCsWithSameName === true && (NPCinfo.isUnique === true || (NPCinfo.name === "Ulfric Stormcloak" || NPCinfo.name === "General Tullius"))) // CWBattleUlfric/Tullius are for some reason not flagged as unique
    {
        for (let i = 0; i < exclusionList.length; i++)
        {
            if (NPCinfo.name.toLowerCase().includes(exclusionList[i]) === true)
            {
                bExcluded = true;
                break;
            }
        ...
        if (bExcluded === false)
        {
            RGtracker.push([NPCinfo.name, NPCinfo.race, NPCinfo.gender, permutationToReturn]);
        }
    }
}
