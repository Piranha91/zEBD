...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.getUserForcedAssignment = function(NPCinfo, forcedNPCAssignments, linkedNPCgroup, assetPackSettings)
    {
        ...
        if (forcedNPC !== undefined && forcedNPC.forcedSubgroups !== undefined)
        {
            ...
            if (Object.keys(forcedAssetPack).length > 0) // Checks if the forced asset pack actually exists in the asset pack list. If the user swaps config files and leaves specific NPC assignments that no longer exist, the downstream loggin function will notify the user.
            {
    ...
    PO.getBlocks = function(NPCrecord, blockList, NPCinfo, logMessage, xelib)
    {
        ...
        {
            writeString += "NPC " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") was blocked by Plugin (" + blockedPlugin.name + ") from: "
        ...
        for (let i = maxPriority; i >= 0; i--)
        {
            for (let j = 0; j < recordsToWrite.length; j++)
            {
                if (recordsToWrite[j].zEBDpriority === i)
                {
                    ...
                    recordsToWrite[j].zEBDhandle = currentRecordHandle;
    ...
    PO.applyPermutation = function(NPCrecordHandle, permutation, formIDdict, bUpdateHeadPartNames, bForwardWNAM_AAs, extraAAexclusions, modifiedWNAMs, xelib, copyToPatch, patchFile)
    {
        let keywordFormID = "";

        for (let i = 0; i < permutation.templates.length; i++)
        {
            // if the current template is a WNAM, forward AAs if necessary
            if (permutation.templates[i].zEBDPosition === "WNAM")
            {
                let additionalAAs = [];
                let additionalAA_EDIDS = [];
                let additionalSlots = [];
                getWNAMextraAAs(NPCrecordHandle, extraAAexclusions, xelib, additionalAAs, additionalAA_EDIDS, additionalSlots); // returns array of handles corresponding to the current NPC's extra WNAM\Armature records
                if (bForwardWNAM_AAs === true && additionalAAs.length > 0)
                {
                    let currentWNAM = xelib.GetElement(permutation.templates[i].zEBDhandle, "", false);
                    let copiedWNAM = null;
                    let orig_EDID = xelib.GetValue(currentWNAM, "EDID");
                    let new_EDID = orig_EDID + "_" + additionalAA_EDIDS.join("_");

                    if (modifiedWNAMs[new_EDID] === undefined)
                    {
                        copiedWNAM = xelib.CopyElement(currentWNAM, patchFile, true);
                        xelib.SetValue(copiedWNAM, "EDID", new_EDID);
                        for (let j = 0; j < additionalAAs.length; j++)
                        {
                            xelib.AddArrayItem(copiedWNAM, "Armature", "", additionalAAs[j]);
                        }
                        additionalSlots = Aux.getArrayUniques(additionalSlots);
                        for (let j = 0; j < additionalSlots.length; j++)
                        {
                            xelib.SetFlag(copiedWNAM, "BOD2\\First Person Flags", additionalSlots[j], true);
                        }
                        
                        modifiedWNAMs[new_EDID] = copiedWNAM;
                    }
                    else
                    {
                        copiedWNAM = modifiedWNAMs[new_EDID];
                    }
                    
                    xelib.SetLinksTo(NPCrecordHandle, "WNAM", copiedWNAM);
                }
                else
                {
            ...
            else if (permutation.templates[i].zEBDPositionisArray === true)
            {
                xelib.AddElementValue(NPCrecordHandle, permutation.templates[i].zEBDPosition + "\\.", permutation.templates[i].zEBDformID);
            }
...
function getWNAMextraAAs(NPCrecordHandle, extraAAexclusions, xelib, additionalFormIDs, additionalEDIDS, additionalSlots)
{
    let WNAM = xelib.GetLinksTo(NPCrecordHandle, "WNAM");
    if (WNAM === 0)
    {
        return additionalFormIDs;
    }

    let AAs = xelib.GetElements(WNAM, "Armature")

    for (let i = 0; i < AAs.length; i++)
    {
        let AA = xelib.GetLinksTo(WNAM, "Armature\\[" + i + "]");
        let bSkipThisAA = false;

        for (let j = 0; j < extraAAexclusions.length; j++)
        {
            let exclPath = extraAAexclusions[j].path;
            let exclValues = extraAAexclusions[j].values;

            let currentVal = xelib.GetValue(AA, exclPath).toLowerCase();
            let valSplit = currentVal.split('\\');
            currentVal = valSplit[valSplit.length - 1];

            if (exclValues.includes(currentVal))
            {
                bSkipThisAA = true;
                break;
            }
        ...
        if (bSkipThisAA == false)
        {
            additionalFormIDs.push(xelib.GetHexFormID(AA));
            additionalEDIDS.push(xelib.EditorID(AA));
            if (xelib.HasElement(AA, "BOD2\\First Person Flags") === true) // this check is necessary because some NPCs have WNAM SkinNakedBeast, which includes HorseAA, which has BODT instead of BOD2. 
            {
                additionalSlots.push(...xelib.GetEnabledFlags(AA, "BOD2\\First Person Flags"));
            }
        }      
    }
