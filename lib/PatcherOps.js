...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.assignNPCheight = function(NPCrecordHandle, NPCinfo, bEnableConsistency, consistencyRecords, heightSettings, userForcedAssignment, bChangeNonDefaultHeight, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCheights)
    {
        let distMode;
        let lowerBound;
        let upperBound;
        ...
        // if NPC assignments are linked by name, check if the current NPC has already been matched and return its height if so
        if (bLinkNPCsWithSameName === true && NPCinfo.isUnique === true)
        {
            heightString = findInfoInlinkedNPCdata(NPCinfo, linkedNPCheights)
        ...
        // consistency
        if (bEnableConsistency === true && NPCinfo.consistencyIndex >= 0 && consistencyRecords[NPCinfo.consistencyIndex].height !== undefined && bHeightForced === false)
        {
            bGenerateRandom = false;
        ...
        updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, heightString, LinkedNPCNameExclusions, linkedNPCheights);
    ...
    PO.choosePermutation = function(NPCrecordHandle, NPCinfo, permutationsByRaceGender, consistencyRecords, bEnableConsistency, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCpermutations, LinkedNPCNameExclusions, attributeCache, logMessage)
    {
        ...
        if (bLinkNPCsWithSameName === true && NPCinfo.isUnique === true)
        {
            permutationToReturn = findInfoInlinkedNPCdata(NPCinfo, linkedNPCpermutations)
        ...
        { 
            addAssignedNPCtoPermutation(originalpermutations, permutationToReturn, NPCinfo);           
            updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, LinkedNPCNameExclusions, linkedNPCpermutations);
            return permutationToReturn; 
        }
        ...
        else 
        {
            ...
            updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, LinkedNPCNameExclusions, linkedNPCpermutations) // store for linking same-named NPCs
...
function findInfoInlinkedNPCdata(NPCinfo, linkedNPCpermutations)
{
    ...
    for (let i = 0; i < linkedNPCpermutations.length; i++)
    {
        if (linkedNPCpermutations[i][0] === NPCinfo.name && linkedNPCpermutations[i][1] === NPCinfo.race && linkedNPCpermutations[i][2] === NPCinfo.gender)
        {
            return linkedNPCpermutations[i][3];
        }
    }
    return undefined;
}

function updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, permutationToReturn, exclusionList, linkedNPCpermutations)
{
    let bExcluded = false;
    if (bLinkNPCsWithSameName === true && (NPCinfo.isUnique === true || (NPCinfo.name === "Ulfric Stormcloak" || NPCinfo.name === "General Tullius"))) // CWBattleUlfric/Tullius are for some reason not flagged as unique
    {
        ...
        if (bExcluded === false)
        {
            linkedNPCpermutations.push([NPCinfo.name, NPCinfo.race, NPCinfo.gender, permutationToReturn]);
        }
    }
}
