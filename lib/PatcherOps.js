...
function chooseRandomPermutation(permutations, choiceArray, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    // new vars
    let bConsistencyMorphExists = consistencyRecords[NPCinfo.formID] !== undefined && consistencyRecords[NPCinfo.formID].assignedMorphs !== undefined;
    ...
    while (bValidChoice === false && choiceArray.length > 0)
    {
        ...
        if (bValidChoice === true)
        {
            if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false)
            {
                ...
                chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
                ...
                if ((bConsistencyMorphExists === false && chosenMorph !== undefined) || (bConsistencyMorphExists === true && chosenMorph.morphs === consistencyRecords[NPCinfo.formID].assignedMorphs))
                {
                ...
                else if (bConsistencyMorphExists === true && chosenMorph.morphs !== consistencyRecords[NPCinfo.formID].assignedMorphs)
                {
                ...
                {
                    chosenMorph2 = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, consistencyRecords, undefined, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
...
function drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, bAssetsForcedByUser, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, xelib, logMessage, attributeCache)
{
    ...
    if (bEnableConsistency === true && bNPCfoundInConsistencyRecords === true)
    {
        ...
        else if (permutationToReturn !== undefined && bPermutationValidForCurrentNPC(NPCrecordHandle, permutationToReturn, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) === true)
        {
            ...
            if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false)
            {
                let chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
                
                // if the morph is valid and there is no consistency morph, return the permutation and morph

                if (chosenMorph !== undefined && consistencyRecords[NPCinfo.formID] === undefined)
                {
                    assignedBodyGen[NPCinfo.formID] = chosenMorph;
                }

                // if the drawn morph is not the same one as in the consistency records, warn the user
                else if (consistencyRecords[NPCinfo.formID] !== undefined && consistencyRecords[NPCinfo.formID].assignedMorphs !== undefined && consistencyRecords[NPCinfo.formID].assignedMorphs !== chosenMorph.morphs)
                {
                    ...
                    else if (BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, undefined, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage) !== undefined) // true if there are morphs available without restrictions from consistency permutation
                    {
...
function findPermutationFromConsistency(permutations, NPCAssignment, userForcedAssignment)
{
    ...
    for (let i = 0; i < permutations.length; i++)
    {
        ...
        if (assignmentComparatorPack === NPCcomparatorPack && assignmentComparatorID === NPCcomparatorID)
        {
            ...
            if (userForcedAssignment !== undefined && userForcedAssignment.forcedAssetPack !== undefined)
            {
