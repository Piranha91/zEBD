...
module.exports = function(logDir, fh, xelib)
{
    ...
    /*
    PO.linkFlattenedRequiredSubgroups = function(assetPackSettings) // replaced subgroup names within requiredSubgroups with pointers to the actual subgroup
    {
        for (let i = 0; i < assetPackSettings.length; i++)
        {
            for (let j = 0; j < assetPackSettings[i].flattenedSubgroups.length; j++)
            {
                ...
                {
                    Aux.linkRequiredSubgroups(assetPackSettings[i].flattenedSubgroups[j][k], assetPackSettings[i]);
                }
            }
        }
    }*/
    ...
    PO.choosePermutation_BodyGen = function(NPCrecordHandle, NPCinfo, assetPackSettings, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, consistencyRecords, bEnableConsistency, bEnableBodyGenIntegration, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCpermutations, linkedNPCbodygen, NPClinkGroup, aliasList, attributeCache, logMessage)
    {
        ...
        let assetPackSettings_mutable = angular.copy(assetPackSettings);
...
//FIX THIS LATER            userForcedSubgroupsApplied = filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bAssetsForcedByUser, logMessage, NPCinfo, bAllowOtherRGCombos);
...
//FIX THIS LATER             }
    ...
    PO.applyPermutation = function(NPCrecordHandle, permutation, formIDdict, bUpdateHeadPartNames, xelib, copyToPatch, categorizedRecords)
    {
        let keywordFormID = "";
        ...
        for (let i = 0; i < permutation.templates.length; i++)
        {
            ...
            {
                xelib.AddElementValue(NPCrecordHandle, permutation.templates[i].zEBDPosition + "\\.", getFormIDFromLinkTo(permutation.templates[i], categorizedRecords));
            }
...
function generateSubgroupChoiceList(assetPackSettings, gender)
{
    ...
    for (let i = 0; i < assetPackSettings.length; i++)
    {
        ...
        {
            choiceList.push(...generateChoiceListFromSubgroupList(assetPackSettings[i].flattenedSubgroups));
...
function generateChoiceListFromSubgroupList(flattenedSubgroups)
{
    let subgroupList = [];
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
        subgroupList.push(...Aux.generateSubgroupArrayByMultiplicity(flattenedSubgroups[i]));
    }
    return subgroupList;
}

function filterRequiredSubgroups(flattenedSubgroups, requirements, index)
{
    let validSubgroups = [];
    if (requirements[index] !== undefined)
    {
        for (let i = 0; i < requirements[index].length; i++)
        {
            for (let j = 0; j < flattenedSubgroups[index].length; j++)
            {
                if (flattenedSubgroups[index][j].id === requirements[index][i].id)
                {
                    validSubgroups.push(flattenedSubgroups[index][j]);
                }
            }
        }
    }
    ...
    {
        flattenedSubgroups[index] = validSubgroups;
    }
}

function removeSubgroupsBySource(requirements, sourceSubgroup) // removes subgroups from the required subgroup list if their source is sourceSubgroup
{
    for (let i = 0; i < requirements.length; i++)
    {
        for (let j = 0; j < requirements[i].length; j++)
        {
            if (requirements[i][j].source === sourceSubgroup)
            {
                requirements[i].splice(j, 1);
                j--;
            }
        }
    }
...
function filterExcludedSubgroups(flattenedSubgroups, exclusions, index)
{
    for (let i = 0; i < flattenedSubgroups[index].length; i++)
    {
        for (let j = 0; j < exclusions.length; j++)
        {
            if (exclusions[j].id === flattenedSubgroups[index][i].id)
            {
                flattenedSubgroups[index].splice(i, 1);
                i--;
            }
        }
    }
...
function addRequiredSubgroupsToPermutation(currentSubgroup, permutation)
{
    ...
    for (let index of Object.keys(currentSubgroup.requiredSubgroups))
    {
        ...
        if (permutation.requiredSubgroups[index] === undefined) // if no required subgroups exist at this index, simply add the ones from this subgroup
        {
            ...
            {
                new_reqs[index].push({"id":currentSubgroup.requiredSubgroups[index][j], "source": currentSubgroup.id});
                req_Added = true;
            }   
        }
...
function bPermutationCompatibleWithNewRequiredSubgroups(currentSubgroup, permutation)
{
    let bMatched = false;
    for (let index of Object.keys(currentSubgroup.requiredSubgroups))
    {
        if (permutation.subgroups[index] !== undefined)
        {
            bMatched = false;
            for (let i = 0; i < currentSubgroup.requiredSubgroups[index].length; i++)
            {
                if (currentSubgroup.requiredSubgroups[index][i] === permutation.subgroups[index].id)
                {
                    bMatched = true; break;
                }
            }
            if (bMatched === false)
            {
                return false; // the subgroup requires a different subgroup at position "index" than the one currently in the permutation
            }
        }
    }
...
function addExcludedSubgroupsToPermutation(currentSubgroup, permutation)
{
    ...
    {
        permutation.excludedSubgroups.push({"id": currentSubgroup.excludedSubgroups[i], "source": currentSubgroup.id});
    }
}

function recalculateRequiredExcludedSubgroups(permutation, index)
{
    permutation.requiredSubgroups = {};

    for (let i = 0; i < index; i++)
    {
        addRequiredSubgroupsToPermutation(permutation.subgroups[index], permutation);
        addExcludedSubgroupsToPermutation(permutation.subgroups[index], permutation);
    }
}

function bAllRequiredSubgroupsAtIndexTried(permutation, subgroupsTried, index)
{
    let requiredSubgroups = permutation.requiredSubgroups[index];
    let sTried = subgroupsTried[index];
    let bMatched = false;

    for (let i = 0; i < requiredSubgroups.length; i++)
    {
        bMatched = false;
        for (let j = 0; j < sTried.length; j++)
        {
            if (requiredSubgroups[i].id === sTried)
            {
                bMatched = true;
                break;
            }
...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    let permutation = {};
    let flattenedSubgroups_backup = [];
    let subgroupChoiceListPrimary = [];
    let subgroupChoiceListSecondary = [];
    let requirementTracker_Bak = {};
    let initSubgroup = {};
    let currentSubgroup = {};
    let chosenAssetPack;
    ...
    let exitFlag = false;
    let bChooseNewSeed = false;
    let subgroupsTriedAtIndex = {};
    let maxPermutationsSince = {};
    ...
    while (bPermutationCreated === false)
    {
        ...
        while (bValidChoice === false)
        {
            ...
            if (subgroupChoiceListPrimary.length === 0)
            {
                exitFlag = true;
                break;
            }
            ...
            for (let i = 0; i < assetPackSettings.length; i++)
            {
                if (assetPackSettings[i].groupName === initSubgroup.parentAssetPack)
                {
                    ...
                    flattenedSubgroups_backup = angular.copy(chosenAssetPack.flattenedSubgroups); // way want to optimize out angular.copy
                    break;
                }
            }

            if (bSubgroupValidForCurrentNPC(NPCrecordHandle, initSubgroup, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) === false)
            {               
            ...
            else
            {
                ...
                permutation.emptyFlagAR = true; // flags compatibilization function that this is a newly formed permutation rather than one whose allowedRaces have been trimmed to zero.
                addRequiredSubgroupsToPermutation(initSubgroup, permutation);
                addExcludedSubgroupsToPermutation(initSubgroup, permutation);

                // initialize placeholders for subgroups
                for (let i = 0; i < chosenAssetPack.subgroups.length; i++)
                {
                    permutation.subgroups.push(undefined);
                }
        ...
        if (exitFlag === true) // no seed groups resulted in a valid permutation
        {
            logMessage("No permutations were able to be generated for NPC "+ NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ")");
            break;
        }
        ...
        {
            bValidChoice = false;
            requirementTracker_Bak = angular.copy(permutation.requiredSubgroups); // may want to optimize this later to avoid angular.copy
            filterRequiredSubgroups(chosenAssetPack.flattenedSubgroups, permutation.requiredSubgroups, i);
            filterExcludedSubgroups(chosenAssetPack.flattenedSubgroups, permutation.excludedSubgroups, i); 

            subgroupsTriedAtIndex[i] = [];

            // get the max possible permutations to try since the previous indices
            maxPermutationsSince[i] = {};
            let tmp = 1;
            for (let index = i; index >= 0; index--)
            {
                if (index === initSubgroup.topLevelIndex)
                {
                     // Do nothing - only one choice for the seed subgroup so tmp stays the same; Maintaining this scheme for code readability.
                }
                
                else if (permutation.requiredSubgroups[index] === undefined)
                {
                    tmp *= chosenAssetPack.flattenedSubgroups[index].length;
                }
                
                else
                {
                    tmp *= permutation.requiredSubgroups[index].length;
                }
                maxPermutationsSince[i][index] = tmp;
            }
            //

            if (i === initSubgroup.topLevelIndex)
            {
                permutation.subgroups[i] = initSubgroup; // no need to add requiredSubgroups because that was handled during seeding.
                if (subgroupsTriedAtIndex[i].includes(initSubgroup.id) === false)
                {
                    subgroupsTriedAtIndex[i].push(initSubgroup.id);
                }
                bValidChoice = true;
            }
            else
            {
                while (bValidChoice === false && chosenAssetPack.flattenedSubgroups[i].length > 0)
                {
                    bSubgroupCompatibleWithPermutation = false;
                    bSubgroupCompatibleWithNPC = false;

                    subgroupChoiceListSecondary = Aux.generateSubgroupArrayByMultiplicity(chosenAssetPack.flattenedSubgroups[i]);

                    currentSubgroup = subgroupChoiceListSecondary[Math.floor(Math.random() * subgroupChoiceListSecondary.length)];

                    if (subgroupsTriedAtIndex[i].includes(currentSubgroup.id) === false)
                    {
                        subgroupsTriedAtIndex[i].push(currentSubgroup.id);
                    }

                    if (bPermutationCompatibleWithNewRequiredSubgroups(currentSubgroup, permutation) === true) // checks to make sure the permutation doesn't already have an incompatible subgroup at a position expected by the new subgroup's requireSubgroups
                    {
                        bSubgroupCompatibleWithPermutation = addRequiredSubgroupsToPermutation(currentSubgroup, permutation); // merges the requiredSubgroups from the permutation and the new subgroup, or returns false if they're mutually incompatible
                    }

                    // check if subgroup is valid for current NPC HERE (if current subgroup fails, remove from BOTH primary and secondary list because it will fail regardless of which permutation it's in)
                    if (bSubgroupCompatibleWithPermutation === true)
                    {
                        bSubgroupCompatibleWithNPC = bSubgroupValidForCurrentNPC(NPCrecordHandle, currentSubgroup, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache);
                    }

                    if (bSubgroupCompatibleWithPermutation === false || bSubgroupCompatibleWithNPC === false)
                    {
                        Aux.stripSubgroupInstancesFromList(currentSubgroup, chosenAssetPack.flattenedSubgroups[currentSubgroup.topLevelIndex]);
                        permutation.requiredSubgroups = angular.copy(requirementTracker_Bak); // undo modifications from addRequiredSubgroupsToPermutation()
                    }
                    else
                    {
                        // if the current subgroup survived the above checks, add it to the permutation
                        permutation.subgroups[i] = currentSubgroup;
                        addExcludedSubgroupsToPermutation(currentSubgroup, permutation);
                        bValidChoice = true;
                    }
                }
            }

            if (bValidChoice === false) // if all subgroups at the current index fail to combine with the previously chosen subgroups, get rid of the last chosen subgroup and try again.
            {
                //abstract everything below
                while (backtrack(subgroupsTriedAtIndex, maxPermutationsSince, i) === true)
                {
                    chosenAssetPack.flattenedSubgroups[i] = flattenedSubgroups_backup[i].slice(); // restore all subgroups in this top-level subgroup
                    i --; 
                    if (i >= 0)
                    {
                            currentSubgroup = permutation.subgroups[i];
                            Aux.stripSubgroupInstancesFromList(currentSubgroup, chosenAssetPack.flattenedSubgroups[i]);
                            recalculateRequiredExcludedSubgroups(permutation, i);
                    }
                    else
                    {
                            Aux.stripSubgroupInstancesFromList(initSubgroup, chosenAssetPack.flattenedSubgroups[initSubgroup.topLevelIndex]);
                ...
                if (i >= 0)
                {
                    i--; // jump back one more to compensate for the fact that the next iteration of the for loop increments forward by one, achieving the desired -1 decrement to the index during the next for loop iteration
                }

                /*
                chosenAssetPack.flattenedSubgroups[i] = flattenedSubgroups_backup[i].slice(); // restore all subgroups in this top-level subgroup
                i--; // go to the previous subgroup
                if (i === initSubgroup.topLevelIndex) // if the previous subgroup was the seed subgroup, skip back one more
                {
                    i--;
                }
                
                if (i >= 0) // if skipping back doesn't go out of bounds, remove the currently chosen subgroup and try a different one
                {
                    currentSubgroup = permutation.subgroups[i];
                    Aux.stripSubgroupInstancesFromList(currentSubgroup, chosenAssetPack.flattenedSubgroups[i]);
                    recalculateRequiredExcludedSubgroups(permutation, i);
                }
                else // if skipping back DOES go out of bounds, pick a different seed subgroup
                {
                    Aux.stripSubgroupInstancesFromList(initSubgroup, chosenAssetPack.flattenedSubgroups[initSubgroup.topLevelIndex]);
                    break;
                }

                if ((permutation.requiredSubgroups[i] === undefined && subgroupsTriedAtIndex[i].length === flattenedSubgroups_backup[i].length) || permutation.requiredSubgroups[i] !== undefined && bAllRequiredSubgroupsAtIndexTried(permutation, subgroupsTriedAtIndex, i) === true) // if all subgroups at the current index have been tried (or all subgroups conforming to the current index's requiredSubgroups), back up another index
                {
                    chosenAssetPack.flattenedSubgroups[i] = flattenedSubgroups_backup[i].slice(); // restore all subgroups in this top-level subgroup
                    i--;
        ...
        // once all top-level subgroups have been filled, break out of the loop
        if (permutation.subgroups[chosenAssetPack.subgroups.length - 1] !== undefined)
        {
            bPermutationCreated = true; 
        }

        //if function gets here and bPermutationCreated === false, it goes back to picking a new seed subgroup
    }
...
function backtrack(subgroupsTriedAtIndex, maxPermutationsSince, index)
{
    if (subgroupsTriedAtIndex[index].length === maxPermutationsSince[index][index])
    {
        return true;
    }
    else
...
// FIX LATER TO AVOID POSSIBILITY OF INFINITE LOOP
    while (bValidChoice === false)
    {
       permutationToReturn = generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache);
...
function writeTemplateToRecord(recordTemplate, recordHandle, xelib, prependPath, EDIDarray, patchableRaces, RNAMdict, categorizedRecords)
{
    ...
    for (let [element, value] of Object.entries(recordTemplate)) // iterate through each element of the record
    {
        ...
        if (value.$zEBDLinkTo != undefined) // If the element's value points to another recordTemplate, insert that template's formID, which should already have been assigned.
        {
            ...
            xelib.AddElementValue(recordHandle, prependPath + element, getFormIDFromLinkTo(value, categorizedRecords)); // After being written they are assigned a formID, so this should never be undefined
        }
        ...
        else if (element.indexOf("zEBD") === 0 || element.indexOf("$zEBD") === 0) { continue; } // if the element is a reserved zEBD value, don't write it to the plugin. As of 1.8 included $zEBD because angular.equals only ignores keys with $ prefix
        ...
        else if (Aux.isObject(value) === true && value.zEBDformID === undefined) // if the value is an object (including array) but not an zEBD Record itself | Note that an array of flags will also be an object, so handle flags here
        {
            ...
            else if (Array.isArray(value) === true) // if the element value is an array
            {
                ...
                {
                    if (value[i].$zEBDLinkTo != undefined) // if the array element's value points to another recordTemplate, push that template's formID to the array
                    {
                        if (i === 0)
                        {
                        ...
                        xelib.AddArrayItem(recordHandle, prependPath + element, "", getFormIDFromLinkTo(value[i], categorizedRecords));
                    }
                    else // if the array's element value is a not another recordTemplate, recursively run this function on the value
                    {
                        ...
                        else
                        {
                            ...
                            writeTemplateToRecord(value[i], recordHandle, xelib, prependPath + element + "\\[" + i.toString() + "]\\", EDIDarray, patchableRaces, RNAMdict, categorizedRecords); // recursion here
                        }
                    }
                }
            }
            ...
            {
                prependPath += element + "\\";
                writeTemplateToRecord(value, recordHandle, xelib, prependPath, EDIDarray, patchableRaces, RNAMdict, categorizedRecords); // recursion here
...
function bSubgroupValidForCurrentNPC(NPCrecordHandle, subgroup, userForcedSubgroups, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    if (userForcedSubgroups.includes(subgroup.id) === false) // if the user has forced the current subgroup then it is automatically valid so skip the following checks
    {
