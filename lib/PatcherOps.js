...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.choosePermutation_BodyGen = function(NPCrecordHandle, NPCinfo, permutationsByRaceGender, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, consistencyRecords, bEnableConsistency, bEnableBodyGenIntegration, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCpermutations, linkedNPCbodygen, NPClinkGroup, attributeCache, logMessage)
    {
        ...
        if (bSkipSelection === false)
        {
            ...
            // CONSISTENCY:
            permutationToReturn = drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, bAssetsForcedByUser, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, xelib, logMessage, attributeCache);
            if (permutationToReturn != undefined) 
            { 
            ...
            if (bSkipRandomSelection === false)
            {
                ...
                permutationToReturn = chooseRandomPermutation(permutations, choiceArray, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, xelib, logMessage, attributeCache);
            }
        }
...
function chooseRandomPermutation(permutations, choiceArray, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    let validPermutations_withConsistency = []; // cache of valid permutations (for storing permutations that themselves conform to NPC's requirements, but do not pair with any BodyGen morphs). Must be compatible with the BodyGen morph stored in consistency
    ...
    while (bValidChoice === false && choiceArray.length > 0)
    {
        ...
        if (bValidChoice === true && bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false)
        {
            // is the chosen permutation compatible with the consistency morph?
            chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, true, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
            if (chosenMorph !== undefined)
            {
                validPermutations_withConsistency.push([permutationToReturn, chosenMorph]);
            }

            // is the chosen permutation compatible with any BodyGen morph?
            else
            {
                chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, false, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
                if (chosenMorph !== undefined)
                {
                ...
                {
                    validPermutations_withoutBodyGen.push([permutationToReturn, chosenMorph]);
                }
            }

            // if the selected permutation doesn't match with a bodygen (with consistency if enabled), call in invalid and pick again
            if (validPermutations_withConsistency.length === 0) 
            {
                bValidChoice = false;
            }
        }
    ...
    // if a permutation was drawn that is compatible with a bodygen morph (including consistency if applicable), pick that one
    if (bValidChoice === true && validPermutations_withConsistency.length > 0)
    {
        assignedBodyGen[NPCinfo.formID] = validPermutations_withConsistency[1];
        return validPermutations_withConsistency[0];
    }
    ...
    else if (bValidChoice === false && validPermutations_withoutConsistency.length >  0)
    {
        ...
        assignedBodyGen[NPCinfo.formID] = validPermutations_withoutConsistency[1];
        return validPermutations_withoutConsistency[0];
    }

    if (bValidChoice === false && validPermutations_withoutBodyGen.length > 0) // if there are one or more permutations that conform to requirements but don't have an allowed BodyGen, pick the first one that worked and then pick a random BodyGen.
    {
        permutationToReturn = validPermutations_withoutBodyGen[0];
    ...
    else if (bValidChoice === false) // if no permutations conform to NPC's requirements:
    {
        permutationToReturn = undefined;
    }
...
function bPermutationValidForCurrentNPC(NPCrecordHandle, permutation, userForcedSubgroups, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    for (let i = 0; i < permutation.subgroups.length; i++) // loop through all contributing subgroups
    {
        ...
        // "Allowed Attributes"
        tmpFailureStr = "NPC lacks allowed attributes:";
...
function drawPermutationFromConsistency(NPCrecordHandle, permutations, consistencyRecords, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, NPCinfo, bEnableConsistency, bNPCfoundInConsistencyRecords, userForcedSubgroupsApplied, bAssetsForcedByUser, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, xelib, logMessage, attributeCache)
{
    ...
    if (bEnableConsistency === true && bNPCfoundInConsistencyRecords === true)
    {
        ...
        if (permutationToReturn !== undefined && bPermutationValidForCurrentNPC(NPCrecordHandle, permutationToReturn, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) === true)
        {
            ...
            {
                let chosenMorph = BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, false, consistencyRecords, permutationToReturn, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage);
                
                // if the drawn morph is not the same one as in the consistency records, warn the user
                if (consistencyRecords[NPCinfo.formID] !== undefined && consistencyRecords[NPCinfo.formID].assignedMorphs !== undefined && consistencyRecords[NPCinfo.formID].assignedMorphs !== chosenMorph.morphs)
                {
                ...
                else if (BGI.assignMorphs(NPCrecordHandle, bodyGenConfig, BGcategorizedMorphs, NPCinfo, bEnableConsistency, true, consistencyRecords, undefined, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCbodygen, NPClinkGroup, true, attributeCache, logMessage) !== undefined) // true if there are morphs available without restrictions from consistency permutation
                {
        ...
        else if (bAssetsForcedByUser === true)
        {
