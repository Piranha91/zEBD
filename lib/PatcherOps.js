...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    let permutationRequirementTracker = {};
    let permutationExclusionTracker = {};
    ...
    {
//IMPLEMENT THIS LATER        permutationRequirementTracker = {};
//IMPLEMENT THIS LATER        permutationExclusionTracker = {};
        ...
        for (let i = 0; i < chosenAssetPack.flattenedSubgroups.length; i++)
        {
            ...
            {
                chosenSubgroups.push(initSubgroup);
                bValidChoice = true;
            }
            else
            {
                while (bValidChoice === false)
                {
                    ...
                    // CHECK REQUIRED AND EXCLUDED PERMUTATIONS HERE (if currentSubgroup fails, remove ONLY from secondary list)

                    // Run the permutation bCompatibilize functions HERE (if current subgroup fails, remove ONLY from secondary list)
                    ...
                    if (bSubgroupValidForCurrentNPC(NPCrecordHandle, currentSubgroup, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache) === false)
                    {
                        Aux.stripSubgroupInstancesFromList(currentSubgroup, chosenAssetPack.flattenedSubgroups[currentSubgroup.topLevelIndex]);
                        Aux.stripSubgroupInstancesFromList(currentSubgroup, subgroupChoiceListPrimary);
                        Aux.stripSubgroupInstancesFromList(currentSubgroup, subgroupChoiceListSecondary);
                        continue;
                    }

                    // if the current subgroup survived the above checks, add it to the permutation
                    chosenSubgroups.push(currentSubgroup);
                    bValidChoice = true;
                }
        ...
        if (chosenSubgroups.length === chosenAssetPack.flattenedSubgroups.length)
        {
...
// FIX LATER TO AVOID POSSIBILITY OF INFINITE LOOP
    ...
    {
        /*
        arrChoice = Math.floor(Math.random() * subgroupChoiceList.length); // pick a number at random between 0 and # of subgroups.
        choice = choiceArray[arrChoice]
        permutationToReturn = permutations[choice];
        bValidChoice = bSubgroupValidForCurrentNPC(NPCrecordHandle, permutationToReturn, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache);
        ...
        */
       permutationToReturn = generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache);
