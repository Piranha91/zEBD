...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.getBlocks = function(NPCrecord, blockList, NPCinfo, logMessage, xelib)
    {
        ...
        if (overrides.length === 0) // if there are no overrides, check if the root plugin is blocked
        {
            for (let j = 0; j < BlockedPlugins.length; j++)
            {
                if (NPCinfo.rootPlugin === BlockedPlugins[j].name)
                {
                    writeString += "NPC " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") was blocked by Plugin (" + NPCinfo.rootPlugin + ") from: "
                    if (BlockedPlugins[j].bBlockAssets === true)
                    {
                        blockInfo.assets = true;
                        writeString += "Assets "
