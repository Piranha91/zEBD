        ...
        for (let i = maxPriority; i >= 0; i--)
        {
            for (let j = 0; j < recordsToWrite.length; j++)
            {
                if (recordsToWrite[j].zEBDpriority === i)
                {
                    ...
                    writeTemplateToRecord(recordsToWrite[j], currentRecordHandle, xelib, "", EDIDarray, patchableRaces, RNAMdict, categorizedRecords);
                }
            }
        }
    }
...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    while (bPermutationCreated === false)
    {
        ...
        addRequiredSubgroupsToPermutation(initSubgroup, permutation, chosenAssetPack.flattenedSubgroups);
        ...
        for (let i = 0; i < chosenAssetPack.flattenedSubgroups.length; i++)
        {
            ...
            if (i === initSubgroup.topLevelIndex)
            {
                ...
                //Aux.appendSubgroupToPH(initSubgroup, permutation); // recordkeeping
                PG.appendSubgroupToPH(initSubgroup, permutation);
                bValidChoice = true;
            }
            else
            {
                while (bValidChoice === false && chosenAssetPack.flattenedSubgroups[i].length > 0)
                {
                    ...
                    if (PG.bResolveSubgroupPermutationConflicts(trialSubgroup, trialPermutation, chosenAssetPack.subgroups, bEnableBodyGenIntegration) === false)
                    {
                    ...
                    else
                    {
                        ...
                        //Aux.appendSubgroupToPH(currentSubgroup, permutation); // recordkeeping
            ...
            else if (Array.isArray(value) === true) // if the element value is an array
            {
                for (let i = 0; i < value.length; i++) // iterate through each array element
                {
                    if (value[i].zEBDformID != undefined) // if the array element's value points to another recordTemplate, push that template's formID to the array
                    {
                        ...
                        xelib.AddArrayItem(recordHandle, prependPath + element, "", value.zEBDformID);
                    }
