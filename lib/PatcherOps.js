...
function filterRequiredSubgroups(flattenedSubgroups, requirements, startIndex, bVerboseMode, verboseReport)
{
    if (bVerboseMode === true)
    {
        ...
        {
            verboseReport.push("Filtering subgroups based on the following requirements:");
    ...
    for (let i = startIndex; i < flattenedSubgroups.length; i++)
    {
        ...
        else if (requirements[i] !== undefined && validSubgroups.length === 0)
        {
            if (bVerboseMode === true)
            {
                verboseReport.push("No subgroups at index " + i + " matched the required subgroups for this position. Filtering by required subgroups has failed.");
            }
...
function filterSubgroupsByForceList(assetPacks, userForcedAssignment, bVerboseMode, verboseLog_UserForceList, logMessage)
{
    bFiltered = false;
    let currentAssetPack;
    let forcedSubgroups = {};
    let forcedIndex = -1;
    let verboseProblemFlag = false;
        ...
        if (userForcedAssignment.forcedSubgroups.length > 0)
        {
            ...
            for (let [index, IDs] of Object.entries(forcedSubgroups))
            {
                ...
                if (bForcedSubgroupFoundAtIndex === false)
                {
                    ...
                    verboseLog_UserForceList("None of the User-forced subgroups at position " + index + " (" + IDs.join(", ") + ") were found within the currently available subgroups at this index. This requirement will be ignored.");
                    verboseProblemFlag = true;
                }
            }
        }
    }
    ...
    if (bVerboseMode === true)
    {
        ...
        if (verboseProblemFlag === true)
        {
            verboseLog_UserForceList.push("problem");
        }
    }
...
function filterSubgroupsByForceIf(assetPacks, NPCrecordHandle, bVerboseMode, verboseLog_ForceIfList, logMessage, xelib, attributeCache)
{
    ...
    for (let i = 0; i < assetPacks.length; i++)
    {
        ...
        for (let j = 0; j < currentAssetPack.flattenedSubgroups.length; j++)
        {
            ...
            for (let k = 0; k < currentAssetPack.flattenedSubgroups[j].length; k++)
            {
                ...
                if (matchCount > maxMatchCountForConfig)
                {
                    maxMatchCountForConfig = matchCount;
                }
                if (matchCount > maxMatchCountGlobal)
                {
                    maxMatchCountGlobal = matchCount;
                }
                if (matchCount > matchCountTracker[currentAssetPack.groupName][j].maxMatchCountPerIndex)
                {
                    matchCountTracker[currentAssetPack.groupName][j].maxMatchCountPerIndex = matchCount;
                }
                
                matchCountTracker[currentAssetPack.groupName][j][k] = matchCount;
            }
        }
    ...
    if (bVerboseMode === true)
    {
        ...
        for (let i = 0; i < assetPacks.length; i++)
        {
            ...
            for (let j = 0; j < assetPacks[i].flattenedSubgroups.length; j++)
            {
                ...
                {
                    verboseLog_ForceIfList.push("\t" + assetPacks[i].flattenedSubgroups[j][k].id + ": " + matchCountTracker[assetPacks[i].groupName][j][k]);
                }
            }
        }
    ...
    for (let i = 0; i < assetPacks.length; i++)
    {
        ...
        for (let j = 0; j < currentAssetPack.flattenedSubgroups.length; j++)
        {
            ...
            for (let k = 0; k < currentAssetPack.flattenedSubgroups[j].length; k++)
            {
                if (matchCountTracker[currentAssetPack.groupName][j][k] === matchCountTracker[currentAssetPack.groupName][j].maxMatchCountPerIndex)
                {
            ...
            {
                verboseLog_ForceIfList.push("Replacing " + assetPacks[i].groupName + " at postition " + j + " with the subgroups best-matched by forceIf attributes.");
            }
        }
    }
...
function filterSubgroupsByConsistency(assetPacks, consistencyInfo, NPCinfo, pre_consistency, bVerboseMode, verboseLog_Consistency, logMessage)
{
    let failedMatchArray = [];
    let verboseProblemFlag = false;

    if (consistencyInfo === undefined)
    {
        return false;
    }
    ...
    if (assetPacks.length === 0)
    {
        ...
        if (bVerboseMode === true)
        {
            verboseLog_Consistency.push("None of the available config files match the name in the consistency record. Generating random permutation for this NPC.");
            verboseLog_Consistency.push("problem");
        }
    ...
    if (verboseProblemFlag === true && bVerboseMode === true)
    {
        verboseLog_Consistency.push("problem");
    }
...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedAssignment, NPCinfo, consistencyInfo, bEnableBodyGenIntegration, bVerboseMode, verboseReport, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    let permutation = {};
    ...
    let bFilteredByUserForceList = filterSubgroupsByForceList(assetPackSettings, userForcedAssignment, bVerboseMode, verboseLog_UserForceList, logMessage);
    ...
    let bFilteredByConsistency = filterSubgroupsByConsistency(assetPackSettings, consistencyInfo, NPCinfo, pre_consistency, bVerboseMode, verboseLog_Consistency, logMessage);
    let fallbackStatement = "";
    let verboseFlag = false;

    // verbose logging
    if (verboseLog_Consistency[verboseLog_Consistency.length - 1] === "problem")
    {
        verboseLog_Consistency.splice(verboseLog_Consistency.length - 1, 1);
        verboseFlag = true;
    }
    if (bFilteredByUserForceList[bFilteredByUserForceList.length - 1] === "problem")
    {
        bFilteredByUserForceList.splice(bFilteredByUserForceList.length - 1, 1);
        verboseFlag = true;
    }
    if (bVerboseMode === true)
    {
        verboseReport.push("GENERATING A NEW PERMUTATION");
        ...
        for (let i = 0; i < flattenedSubgroupsSecondary.length; i++)
        {
            ...
            while (bValidChoice === false && flattenedSubgroupsSecondary[i].length > 0)
            {
                ...
                if (bVerboseMode === true)
                {
                    ...
                    verboseReport.push("Choices: " + Aux.formatAssetPacksForVerbose(assetPackSettings, true));
                ...
                {                    
                    if (filterRequiredSubgroups(trialFlattenedSubgroups, trialPermutation.requiredSubgroups, i, bVerboseMode, verboseReport) !== false) // returns false if none of the remaining subgroups are compatible with the filter
                    {
                        ...
                        {
                            verboseReport.push("Subgroup " + currentSubgroup.id + " (or its required subgroups) is not compatible with the current permutation's excluded subgroups.");
                        }
                    }
                    else if (bVerboseMode === true)
                    {
                        verboseReport.push("Subgroup " + currentSubgroup.id + " (or its required subgroups) is not compatible with the current permutation's required subgroups.");
                    }
                }
                else if (bVerboseMode === true)
                {
                    verboseReport.push("Subgroup " + currentSubgroup.id + " is not compatible with the current permutation.");
                }
                ...
                else
                {
                    if (bVerboseMode === true)
                    {
                        verboseReport.push("Removing subgroup " + currentSubgroup.id + " from the available subgroup list");
                    }
            ...
            if (flattenedSubgroupsSecondary[i].length === 0 && bValidChoice === false)
            {
                ...
                else
                {
                    if (bVerboseMode === true)
                    {
                        verboseReport.push("Backtracking to index " + i - 1 + ", removing the current subgroup (" + permutation.subgroups[i-1].id + "), and choosing a different subgroup for this position.");
                    }
    ...
    // decide if verbose log needs to be cleared
    if (bVerboseMode === true && verboseFlag === false)
    {
        verboseReport.length = 0; // this works to clear the array
    }
...
function chooseRandomPermutation(assetPackSettings, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, raceAliases, bVerboseMode, xelib, logMessage, attributeCache, fh, modulePath)
{
    ...
    let verboseReportPath = modulePath + "\\Logs\\Failed asset assignments\\" + NPCinfo.EDID + " (" + NPCinfo.formID + ").txt";
    ...
    {
       permutationToReturn = generatePermutation(assetPackSettings, NPCrecordHandle, userForcedAssignment, NPCinfo, consistencyRecords[NPCinfo.consistencyIndex], bEnableBodyGenIntegration, bVerboseMode, currentVerboseReport, xelib, logMessage, attributeCache);
       if (currentVerboseReport.length > 0)
       {
       ...
       else
       {
        ...
        if (bEnableBodyGenIntegration === true && userBlockedAssignment.bodygen === false && Object.keys(BGcategorizedMorphs[NPCinfo.gender]).length > 0) // gender check to avoid wasting time on NPCs that don't have morphs assigned
        {
            ...
            if (chosenMorph === undefined)
            {
                ...
                {
                    if (bVerboseMode === true && verboseReport.length > 2)
                    {
                        fh.saveTextFile(verboseReportPath, verboseReport.join("\n"));
                    }
