...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.choosePermutation = function(NPCrecordHandle, NPCinfo, permutationsByRaceGender, consistencyRecords, bEnableConsistency, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCpermutations, attributeCache, logMessage)
    {
        ...
        bUserForcedSubgroupsApplied = filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bAssetsForcedByUser, logMessage, NPCinfo, allowOtherRGCombos);
...
function bPermutationValidForCurrentNPC(NPCrecordHandle, permutation, bUserForcedSubgroupsApplied, bForceIfAttributesApplied, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    for (let i = 0; i < permutation.disallowedAttributes.length; i++) 
    {
        if (permutation.topLevelSubgroupsSatisfied.includes(permutation.disallowedAttributes[i].sourceSubgroupTop) === false)
        {
            ...
            { 
                failureModeString = "NPC has disallowed attribute: (" + permutation.disallowedAttributes[i].attribute[0] + ": " + permutation.disallowedAttributes[i].attribute[1];
                if (failureModes[failureModeString] === undefined)
                {
                    failureModes[failureModeString] = 1;
                }
    ...
    for (let i = 0; i < permutation.allowedAttributes.length; i++) 
    {
        if (permutation.topLevelSubgroupsSatisfied.includes(permutation.allowedAttributes[i].sourceSubgroupTop) === false)
        {
            failureModeString = "NPC lacks allowed attribute: (" + permutation.allowedAttributes[i].attribute[0] + ": " + permutation.allowedAttributes[i].attribute[1];
            if (failureModes[failureModeString] === undefined)
            {
                failureModes[failureModeString] = 1;
            }
...
function filterPermutationsByUserForceList(permutations, permutationsByRaceGender, userForcedAssignment, bAssetsForcedByUser, logMessage, NPCinfo, allowOtherRGCombos) // allowOtherRGCombos significantly increases processing time
{
