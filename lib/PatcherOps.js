...
function bPermutationCompatibleWithNewRequiredSubgroups(currentSubgroup, permutation)
{
    ...
    for (let index of Object.keys(currentSubgroup.requiredSubgroups))
    {
        if (permutation.subgroups[index] !== undefined)
        {
            ...
            {
                if (currentSubgroup.requiredSubgroups[index][i].id === permutation.subgroups[index].id)
                {
                    bMatched = true; break;
                }
            }
...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    while (bPermutationCreated === false)
    {
        ...
        while (bValidChoice === false)
        {
            ...
            for (let i = 0; i < assetPackSettings.length; i++)
            {
                if (assetPackSettings[i].groupName === initSubgroup.parentAssetPack)
                {
                    ...
                    flattenedSubgroups_backup = chosenAssetPack.flattenedSubgroups.slice();
                    break;
                }
            }
        ...
        for (let i = 0; i < chosenAssetPack.flattenedSubgroups.length; i++)
        {
            ...
            {
                permutation.subgroups.push(initSubgroup); // no need to add requiredSubgroups because that was handled during seeding.
                bValidChoice = true;
            }
            else
            {
                while (bValidChoice === false)
                {
                    ...
                    // check if there are any subgroups left
                    if (chosenAssetPack.flattenedSubgroups[i].length === 0)
                    {
                        break;
                    }
            ...
            {
                chosenAssetPack.flattenedSubgroups[i] = flattenedSubgroups_backup[i]; // restore all subgroups in this top-level subgroup
                ...
                {
                    Aux.stripSubgroupInstancesFromList(initSubgroup, chosenAssetPack.flattenedSubgroups[i]);
                    break;
                }
            }
        }
