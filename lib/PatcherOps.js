...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    while (bPermutationCreated === false)
    {
        ...
        while (bPermutationCreated === false)
        {
            let i = getNextIndexToFill(permutation);
            bValidChoice = false;
            ...
            while (bValidChoice === false && chosenAssetPack.flattenedSubgroups[i].length > 0)
            {
                ...
                {
                    currentSubgroup = initSubgroup;
                    chosenAssetPack.flattenedSubgroups[initSubgroup.topLevelIndex] = [initSubgroup]; // since the only subgroup that works here is the seed subgroup, make sure that the only subgroup in the array is the seed subgroup so that if it fails, the loop knows to break
                }
                else
                {
            ...
            {
                bPermutationCreated = allSubgroupsFilled(permutation);
            }
        }
    }
