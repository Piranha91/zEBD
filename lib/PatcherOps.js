    ...
    PO.applyPermutation = function(NPCrecordHandle, permutation, formIDdict, bUpdateHeadPartNames, bForwardWNAM_AAs, extraAAexclusions, modifiedWNAMs, xelib, copyToPatch, patchFile)
    {
        ...
        for (let i = 0; i < permutation.templates.length; i++)
        {
            ...
            if (permutation.templates[i].zEBDPosition === "WNAM")
            {
                ...
                getWNAMextraAAs(NPCrecordHandle, extraAAexclusions, xelib, additionalAAs, additionalAA_EDIDS); // returns array of handles corresponding to the current NPC's extra WNAM\Armature records
...
function getWNAMextraAAs(NPCrecordHandle, extraAAexclusions, xelib, additionalFormIDs, additionalEDIDS)
{
