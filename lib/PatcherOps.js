...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.choosePermutation_BodyGen = function(NPCrecordHandle, NPCinfo, permutations, assetPackSettings, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, consistencyRecords, bEnableConsistency, bEnableBodyGenIntegration, userForcedAssignment, userBlockedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCpermutations, linkedNPCbodygen, NPClinkGroup, aliasList, attributeCache, logMessage)
    {
        ...
        if (bSkipSelection === false)
        {
            ...
            // new in 1.9
            assetPackSettings_Filtered = filterValidConfigsForNPC(NPCrecordHandle, NPCinfo, assetPackSettings, userForcedAssignment, NPCinfo, failureModes, xelib, logMessage, attributeCache)
...
function filterValidConfigsForNPC(NPCrecordHandle, NPCinfo, assetPackSettings, userForcedAssignment, NPCinfo, failureModes, xelib, logMessage, attributeCache)
{
    ...
    for (let i = 0; i < assetPackSettings.length; i++)
    {
        ...
        if (userForcedAssignment !== undefined && userForcedAssignment.forcedAssetPack !== "" && userForcedAssignment.forcedAssetPack !== assetPackSettings[i].groupName)
        {
            continue;
        }

        else if (assetPackSettings[i].gender !== NPCinfo.gender)
        {
            continue;
        }
        ...
        else
        {
            ...
            for (let j = 0; j < assetPackSettings[i].flattenedSubgroups.length; j++)
            {
                ...
                for (let k = 0; k < assetPackSettings[i].flattenedSubgroups[j].length; k++)
                {
                    if (forcedSubgroups[j] !== undefined)
                    {
                        if (forcedSubgroups[j].includes(assetPackSettings[i].flattenedSubgroups[j][k].id) === false)
                        {
                            continue;
                        }
                    }

                    else if (bSubgroupValidForCurrentNPC(NPCrecordHandle, assetPackSettings[i].flattenedSubgroups[j][k], NPCinfo, failureModes, xelib, logMessage, attributeCache) === false)
                    {
                        continue;
                    }

                    // this code is only reached if there are forced subgroups which include the current subgroup, or if there are no forced subgroups and the subgroup is compatible with the current NPC
                    newSubgroup = Aux.initializeSubgroup(assetPackSettings[i].flattenedSubgroups[j][k]);
                    Aux.copySubgroupComponents(assetPackSettings[i].flattenedSubgroups[j][k], newSubgroup);
                    tmp.flattenedSubgroups[j].push(newSubgroup);
                }
...
function generateSeedList(assetPackSettings)
{
    let choiceList = [];
    for (let i = 0; i < assetPackSettings.length; i++)
    {
        for (let j = 0; j < assetPackSettings[i].flattenedSubgroups.length; j++)
        {
            for (let k = 0; k < assetPackSettings[i].flattenedSubgroups[j].length; k++)
            {
                choiceList.push(assetPackSettings[i].flattenedSubgroups[j][k]);
            }
        }    
    }
    return choiceList;
    ...
    if (requirements[index] !== undefined)
    {
        for (let i = 0; i < requirements[index].length; i++)
        {
            for (let j = 0; j < flattenedSubgroups[index].length; j++)
            {
                ...
                if (flattenedSubgroups[index][j].containedSubgroupIDs.includes(requirements[index][i]))
                {
...
function filterExcludedSubgroups(flattenedSubgroups, exclusions, index)
{
    for (let i = 0; i < flattenedSubgroups[index].length; i++)
    {
        for (let j = 0; j < exclusions.length; j++)
        {
            ...
            if (flattenedSubgroups[index][i].containedSubgroupIDs.includes(exclusions[j]))
            {
...
function addRequiredSubgroupsToPermutation(currentSubgroup, permutation, flattenedSubgroups)
{
...
function bPermutationCompatibleWithNewRequiredSubgroups(currentSubgroup, permutation)
{
    ...
    for (let index of Object.keys(currentSubgroup.requiredSubgroups))
    {
        if (permutation.subgroups[index] !== undefined)
        {
            ...
            {
                if (permutation.subgroups[index].containedSubgroupIDs.includes(currentSubgroup.requiredSubgroups[index][i])) // .includes instead of === allows matching of higher-level subgroups (e.g. if the required subgroup is BN.M, an id of BN.M.v1 will work)
                {
                    bMatched = true; break;
                }
            }
...
function buildRequiredExcludedSubgroupChain(seedGroup, flattenedSubgroups, lockedTopLevelIndices, permutationsByWeight)
{
    // for each subgroup, check if it excludes the seed subgroup and remove it.
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
        if (Object.keys(lockedTopLevelIndices).includes(i)) // skip permutation indices that have already been selected
        {
            continue;
        }
        ...
        {
            if (flattenedSubgroups[i][j].excludedSubgroups.includes(seedGroup.id))
            {
                flattenedSubgroups[i].splice(j, 1);
                j--;
            }
        }
    }

    // for each subgroup, check if its requirements disallow the current subgroup. If so, remove them.
    let subgroupsAtCurrentIndex = [];
    let trialSubgroup = {};
    let bRequirementMatched = false;
    let removalList = {};
    ...
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
        ...
        for (let j = 0; j < flattenedSubgroups[i].length; j++)
        {
            for (let index of Object.keys(flattenedSubgroups[i][j].requiredSubgroups))
            {
                bRequirementMatched = false;
                subgroupsAtReqIndex = flattenedSubgroups[index];
                for (let k = 0; k < subgroupsAtReqIndex.length; k++)
                {
                    trialSubgroup = subgroupsAtReqIndex[k];
                    if (Aux.isSubgroupBallowedBySubgroupA_sRequiredSubgroups(trialSubgroup, seedGroup, seedGroup.topLevelIndex) === false)
                    {
                        if (removalList[index] === undefined)
                        {
                            removalList[index] = [];
                        }
                        removalList[index].push(Aux.getSubgroupIndexBySubgroupFromArray(trialSubgroup, flattenedSubgroups[index], "byReference"));
                    }
                }
            }
        }
    ...
    let removalCount = 0;
    for (let [index, removalIndexArray] of Object.entries(removalList))
    {
        removalCount = 0; // adjusts for subgroup indices shifting when an earlier subgroup gets removed
        for (let i = 0; i < removalIndexArray.length; i++)
        {
            flattenedSubgroups[index].splice(removalIndexArray[i] - removalCount, 1);
            removalCount++;
        }
    }

    // if the above process has resulted in an empty index within flattenedSubgroups (e.g. all possible subgroups were eliminated), halt this recursion
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
    ...
    // for each remaining subgroup, repeat this process recursively to see, if they're chosen as the next subgroup, if they have a path to building a valid permutation
    let currentFlattenedSubgroups = [];
    let currentLockedTopLevelIndices = [];
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
        if (flattenedSubgroups[i].length > 1)
        {
            for (let j = 0; j < flattenedSubgroups[i].length; j++)
            {
                currentFlattenedSubgroups = flattenedSubgroups.slice();
                currentFlattenedSubgroups[i] = [flattenedSubgroups[i][j]];

                currentLockedTopLevelIndices = lockedTopLevelIndices.slice();
                currentLockedTopLevelIndices.push(i);

                buildRequiredExcludedSubgroupChain(flattenedSubgroups[i][j], currentFlattenedSubgroups, currentLockedTopLevelIndices, permutationsByWeight);
            }
        }
    }

    // check if each flattenedSubgroup position is filled with a single subgroup
    let bSingleOccupancy = true;
    for (let i = 0; i < flattenedSubgroups.length; i++)
    {
        if (flattenedSubgroups[i].length !== 1)
        {
            bSingleOccupancy = false;
            break;
        }
    }

    // if so, compute the total probability weight of the current permutation and store the permutation/weight combo.
    if (bSingleOccupancy === true)
    {
        let probability = 1;
        let subgroupArray = [];
        for (let i = 0; i < flattenedSubgroups.length; i++)
        {
            probability *= flattenedSubgroups[i][0].probabilityWeighting;
            subgroupArray.push(flattenedSubgroups[i][0]);
        }

        permutationsByWeight.push({"subgroups": subgroupArray, "probabilityWeighting": probability});
    }
}

function generatePermutation2(assetPackSettings, bEnableBodyGenIntegration)
{
    let permutations = PG.generateInlinePermutations(assetPackSettings, bEnableBodyGenIntegration);
...
function selectPermutation(permutations)
{
    if (permutations.length === 0)
    {
        return undefined;
    ...
    for (let i = 0; i < permutations.length; i++)
    {
        for (let j = 0; j < permutations[i].probabilityWeighting; j++)
        {
            choiceArray.push(i);
        }
    }
    ...
    let selectedPermutationIndex = choiceArray[choice];

    let toReturn = permutations[selectedPermutationIndex];
    permutations.splice(selectedPermutationIndex, 1); // remove the chosen permutation so that if it fails, it doesn't get re-tried.
...
function generatePermutation(assetPackSettings, NPCrecordHandle, userForcedSubgroupsApplied, NPCinfo, failureModes, bEnableBodyGenIntegration, xelib, logMessage, attributeCache) // assetPackSettings here is a deep copy and can be pruned.
{
    ...
    let seedSubgroupList = generateSeedList(assetPackSettings);
    let seedSubgroupsWeighted = [];
    let chosenSeedIndex = 0;

    while (seedSubgroupList.length > 0 && bPermutationCreated === false)
    {
        seedSubgroupsWeighted = [];
        for (let i = 0; i < seedSubgroupList.length; i++)
        {
            for (let j = 0; j < seedSubgroupList[i].probabilityWeighting; j++)
            {
                seedSubgroupsWeighted.push(seedSubgroupList[i]);
            }
        }

        // pick a nucleating subgroup
        initSubgroup = seedSubgroupsWeighted[Math.floor(Math.random() * seedSubgroupsWeighted.length)];
        for (let i = 0; i < assetPackSettings.length; i++)
        {
        ...
        let permutationsByProbability = [];
        buildRequiredExcludedSubgroupChain(initSubgroup, chosenAssetPack.flattenedSubgroups, [initSubgroup.topLevelIndex], permutationsByProbability);

        if (permutationsByProbability.length === 0)
        {
            chosenSeedIndex = Aux.getSubgroupIndexBySubgroupFromArray(initSubgroup, seedSubgroupList, "byReference");
            seedSubgroupList.splice(chosenSeedIndex, 1);
            continue;
        }

        let choiceArray = [];
        for (let i = 0; i < permutationsByProbability.length; i++)
        {
            for (let j = 0; j < permutationsByProbability[i].probabilityWeighting; j++)
            {
                choiceArray.push(permutationsByProbability[i].subgroups);
            }
        }

        let chosenPermutationSubgroups = choiceArray[Math.floor(Math.random() * choiceArray.length)];
        permutation = new permutationHolder(chosenAssetPack.groupName);
        permutation.emptyFlagAR = true;
        for (let i = 0; i < chosenPermutationSubgroups.length; i++)
        {
            let tmpSubgroup = angular.copy(chosenPermutationSubgroups[i]);
            if (PG.bResolveSubgroupPermutationConflicts(tmpSubgroup, permutation, chosenAssetPack.subgroups, bEnableBodyGenIntegration) === false)
            {
                let debug; // this should never be reached.
            }
            else
            {
                PG.appendSubgroupToPH(tmpSubgroup, permutation);
            }
        }
        bPermutationCreated = true;
    }

    if (bPermutationCreated === false)
    {
        return undefined;
    }
...
function allSubgroupsFilled(permutation)
{
    for (let i = 0; i < permutation.subgroups.length; i++)
    {
        if (permutation.subgroups[i] === undefined)
        {
            return false;
        }
    }
...
function getNextIndexToFill(permutation)
{
    // first check if there are any required subgroups left unfilled
    for (let index of Object.keys(permutation.requiredSubgroups))
    {
        if (permutation.subgroups[index] === undefined)
        {
            return index;
    ...
    // then check if there are any unfilled subgroups
    for (let i = 0; i < permutation.subgroups.length; i++)
    {
        if (permutation.subgroups[i] === undefined)
        {
...
function chooseRandomPermutation(assetPackSettings, assignedBodyGen, bodyGenConfig, BGcategorizedMorphs, bEnableBodyGenIntegration, bEnableConsistency, consistencyRecords, userForcedAssignment, userBlockedAssignment, NPCrecordHandle, userForcedSubgroupsApplied, bLinkNPCsWithSameName, linkedNPCbodygen, NPClinkGroup, NPCinfo, failureModes, raceAliases, xelib, logMessage, attributeCache)
{
    ...
    while (bValidChoice === false)
    {
       ...
       permutationToReturn = selectPermutation(permutations);
