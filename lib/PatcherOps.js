...
function filterSubgroupsByPreviousPermutation(tracker, assetPackSettings, previousIterationOutput, subgroupChoiceListPrimary, verboseReport, out_writeVerbose)
{
    ...
    {
        logVerbose("\tAll possible seed subgroups from the current set of available subgroups have been tried. Attempting to remove a different subgroup from the available list.", verboseReport, out_writeVerbose);
        tracker.chosenUniqueSeeds = [];

        // restore previously removed subgroup if it exists
        if (tracker.splicedSubgroupCoordinates.length > 0)
        {
            assetPackSettings[tracker.splicedSubgroupCoordinates[0]].flattenedSubgroups[tracker.splicedSubgroupCoordinates[1]].splice(tracker.splicedSubgroupCoordinates[2], 0, ...tracker.splicedSubgroup); // spread operator because array.splice returns an array (in this case an array of length 1)
            logVerbose("\tRestoring previously removed subgroup " + tracker.splicedSubgroup.id + " to " + tracker.packToTrim.groupName, verboseReport, out_writeVerbose, verboseReport, out_writeVerbose);
        }
        ...
        if (tracker.removedSubgroupIndex === previousIterationOutput.generatedPermutationSubgroups[tracker.removedSubgroup_ParentPermutationIndex].length) 
        {
            tracker.removedSubgroupIndex = 0;
            tracker.removedSubgroup_ParentPermutationIndex++;
            logVerbose("\tNo subgroups remain available to remove in asset pack " + tracker.packToTrim.groupName + ". Moving to the next asset pack.", verboseReport, out_writeVerbose);
        }
        // if there are no more previously generated permutations to operate on
        if (tracker.removedSubgroup_ParentPermutationIndex === previousIterationOutput.generatedPermutationSubgroups.length)
        {
            logVerbose("\tNo asset packs from which subgroups can be subtracted remain. No more unique permutations can be generated.", verboseReport, out_writeVerbose);
        ...
        tracker.packToTrimIndex = Aux.getAssetPackIndexByName(previousIterationOutput.generatedPermutationAssetPacks[tracker.removedSubgroup_ParentPermutationIndex], assetPackSettings);
        tracker.packToTrim = assetPackSettings[tracker.packToTrimIndex];
        tracker.subgroupIDtoTrim = previousIterationOutput.generatedPermutationSubgroups[tracker.removedSubgroup_ParentPermutationIndex][tracker.removedSubgroupIndex];

        logVerbose("\tRemoving subgroup " + tracker.subgroupIDtoTrim + " from " + tracker.packToTrim.groupName, verboseReport, out_writeVerbose);

        for (let z = 0; z < tracker.packToTrim.flattenedSubgroups[tracker.removedSubgroupIndex].length; z++)
        {
            if (tracker.packToTrim.flattenedSubgroups[tracker.removedSubgroupIndex][z].id === tracker.subgroupIDtoTrim)
            {
                tracker.splicedSubgroupCoordinates = [tracker.packToTrimIndex, tracker.removedSubgroupIndex, z];
                tracker.splicedSubgroup = tracker.packToTrim.flattenedSubgroups[tracker.removedSubgroupIndex].splice(z, 1);
            }
        }

        tracker.removedSubgroupIndex++;
    }
    else
    {
        logVerbose("\tTrying a different seed subgroup from the current set of available subgroups.", verboseReport, out_writeVerbose);
    }
    return true;
}
...
function initializeRemovedSubgroupTracker()
{
    ...
    tracker.removedSubgroupIndex = 0; // index of the subgroup being removed. If removing the previously generated subgroup at this index reslults in no valid permutations, remove the subgroup at the next index. If all subgroups have been removed, try removing them from the next previously-genereated permutation
    tracker.removedSubgroup_ParentPermutationIndex = 0; // index of the parent permutation whose subgroups are being removed (e.g. if it's the first previously generated permutation, second, etc.)
    ...
    while (bPermutationCreated === false)
    {
        ...
        for (let i = 0; i < flattenedSubgroupsSecondary.length; i++)
        {
            ...
            else if (permutation.subgroups.length === chosenAssetPack.flattenedSubgroups.length)
            {
                ...
                {
                    logVerbose("Permutation " + permutation.nameString + " has been generated previously for this NPC. Attempting to generate a new permutation.", verboseReport, out_writeVerbose);
                    break;
                }
                else
                {
