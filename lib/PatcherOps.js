...
module.exports = function(logDir, fh, xelib)
{
    ...
    PO.writeAssets = function(assetRecords, patchFile, logMessage, patchableRaces, RNAMdict)
    {
        ...
        for (let i = maxPriority; i >= 0; i--)
        {
            ...
            /*
            for (let j = 0; j < categorizedRecords.length; j++)
            {
                if (categorizedRecords[j].zEBDpriority === i)
                {
                    recordCounter++;
                    counter++;
                    basePath = categorizedRecords[j].zEBDSignature + "\\" + categorizedRecords[j].zEBDSignature;
                    currentRecordHandle = xelib.AddElement(patchFile, basePath);
                    writeTemplateToRecord(categorizedRecords[j], currentRecordHandle, xelib, "", EDIDarray, patchableRaces, RNAMdict);
                    if (recordCounter % 100 === 0) { logMessage("Wrote " + recordCounter + " new records")}
                }
            }
            */
...
function writeTemplateToRecord(recordTemplate, recordHandle, xelib, prependPath, EDIDarray, patchableRaces, RNAMdict, categorizedRecords)
{
    ...
    for (let [element, value] of Object.entries(recordTemplate)) // iterate through each element of the record
    {
        ...
        if (value.$zEBDLinkTo != undefined) // If the element's value points to another recordTemplate, insert that template's formID, which should already have been assigned.
        {
            ...
            //linkPointer = value.$zEBDLinkTo;
            //linkedRecord = categorizedRecords[linkPointer.zEBDUniqueID][linkPointer.sourceAssetPack][linkPointer.index];
            //xelib.AddElementValue(recordHandle, prependPath + element, linkedRecord.zEBDformID);
            
            xelib.AddElementValue(recordHandle, prependPath + element, getFormIDFromLinkTo(value, categorizedRecords));
        }
            ...
            else if (Array.isArray(value) === true) // if the element value is an array
            {
                for (let i = 0; i < value.length; i++) // iterate through each array element
                {
                    if (value[i].$zEBDLinkTo != undefined) // if the array element's value points to another recordTemplate, push that template's formID to the array
                    {
                        ...
                        //linkPointer = value[i].$zEBDLinkTo;
                        //linkedRecord = categorizedRecords[linkPointer.zEBDUniqueID][linkPointer.sourceAssetPack][linkPointer.index];
                        //xelib.AddArrayItem(recordHandle, prependPath + element, "", linkedRecord.zEBDformID);
