...
module.exports = function(logDir, fh, xelib)
{
    ...
    {
        let NPCForcedSubgroupsIndex = findNPCfromInfo(forcedNPCAssignments, NPCinfo);// for user-assigned subgroup IDs
        return forcedNPCAssignments[NPCForcedSubgroupsIndex];
    }

    PO.assignNPCheight = function(NPCrecordHandle, NPCinfo, bEnableConsistency, consistencyRecords, heightSettings, userForcedAssignment)
    {
        let distMode;
        let lowerBound;
        let upperBound;
        ...
        if (userForcedAssignment !== undefined && userForcedAssignment.forcedHeight !== undefined)
        {
            return parseFloat(userForcedAssignment.forcedHeight).toFixed(6);
        }
...
function findNPCfromInfo(NPCList, NPCinfo)
{
    ...
    for (let i = 0; i < NPCList.length; i++)
    {
        ...
        if (NPCList[i].rootPlugin === NPCinfo.masterRecordFile && FormIDSigRef === FormIDSigCurrent)
        {
            return i;
        }       
    }
