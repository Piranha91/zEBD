...
module.exports = function()
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, aliasList, attributeCache, logMessage, passConsistencyMessage, passFailureMessage)
    {
        let returnObj = {};
        let assignedMorphs = [];
        let assignedGroups = [];
        ...
        let forwardedRequiredMorphs = []; // for morphs that require other morphs
        ...
        passConsistencyMessage.bIncompatibleWithNPC = false;
        passConsistencyMessage.bIncompatibleWithPermutation = false;
        ...
        // alias NPC as needed
        Aux.setAliasRace(NPCinfo, aliasList, "bodyGen");
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                morphs[assignedGroup] = filterMorphsByForceIf(NPCrecordHandle, NPCinfo, morphs[assignedGroup], assignedGroup, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, {}, attributeCache, xelib, logMessage);
                ...
                if (chosenMorph === undefined)
                {
                    ...
                    else
                    {
                        ...
                        passConsistencyMessage.bIncompatibleWithNPC = true;
                    }
                    break;
                }

                if (bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, assignedGroup, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs, consistencyRecords[NPCinfo.consistencyIndex].assignedGroups, forwardedRequiredMorphs, failureModes, attributeCache, xelib, logMessage) === false) // pass the original categorizedMorphs because morphs can be pruned upstream by 
                {
                    bForcedValid = false;
                    if (bSuppressErrorMessage === false)
                    {
                    ...
                    else
                    {
                        ...
                        else if (NPCincompatibleKeys === 0 && permutationIncompatibleKeys > 0)
                        {
                            ...
                            passConsistencyMessage.bIncompatibleWithPermutation = true;
                        }
                        else if (NPCincompatibleKeys > 0 && permutationIncompatibleKeys > 0) // note: this currently can never be reached because in the interest of speed, bMorphIsValid() returns false as soon as one of the above conditions is met. However, leaving this here in case I decide to change this in a future version.
                        {
                ...
                {
                    forwardedRequiredMorphs.push(...chosenMorph.requiredTemplates);
                }
        ...
        while (bFoundValidCombination === false && combinations.length > 0) // note: The tracked parameter here is combinations
        {
            ...
            forwardedRequiredMorphs = []; // reset the required morph list

            for (let i = 0; i < combination.length; i++)  // choose a morph for each group in the combination
            {
                group = combination[i]; // STRING e.g. "top" or "bottom"
                failureModesByCombination[group] = {};
                selectionList = morphs[group].slice(); // shallow copy to allow pruning

                if (filteredItems.includes(group) === false) // filteredItems simply logs that the given group within the morph list has already been filtered by ForceIF and doesn't need to be filtered again
                {
                    selectionList = filterMorphsByForceIf(NPCrecordHandle, NPCinfo, selectionList, group, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, failureModesByCombination[group], attributeCache, xelib, logMessage);
                ...
                while (bFoundValidMorph === false && selectionList.length > 0) // note: The tracked parameter here is selectionList
                {
                    ...
                    bFoundValidMorph = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, group, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, assignedMorphs, combination, forwardedRequiredMorphs, failureModesByCombination[group], attributeCache, xelib, logMessage);
                    ...
                    if (bFoundValidMorph === true)
                    {
                        ...
                        delete failureModesByCombination[group]; // if the given group within the combination was assigned succesfully, clear its failureModes so that they aren't erroneously reported

                        if (i === combination.length - 1) // if the current group is the last one in the combination, and the morph for this group is valid, then the whole combination is valid.
                        {
                            bFoundValidCombination = true;
                        }
                    }
                ...
                if (selectionList.length === 0) // if there are no valid morphs for the current group within the current combination:
                {
                    ...
                    {
                        if (combinations[j].members.includes(group) === true)
                        {
        ...
        else
        {
            ...
            else
            {
                ...
                for (let [combName, combFailureModes] of Object.entries(failureModesByCombination))
                {
                    passFailureMessage.message += Aux.formatFailureModeString(combFailureModes, "morph", "combination " + combName) + "\n";
                }
            }
            Aux.revertAliasRace(NPCinfo);
            return undefined;
    ...
    // expects BGI.formatMorphDescriptors to have been called first
    BGI.getDescriptorsByGroup = function(templateList)
    {
        let descriptorsByCategory = {};

        for (let i = 0; i < templateList.length; i++)
        {
            for (let j = 0; j < templateList[i].groups.length; j++)
            {
                if (descriptorsByCategory[templateList[i].groups[j]] === undefined)
                {
                    descriptorsByCategory[templateList[i].groups[j]] = {};
                }

                for (let k = 0; k < templateList[i].descriptors.length; k++)
                {
                    if (Object.keys(descriptorsByCategory[templateList[i].groups[j]]).includes(templateList[i].descriptors[k].category) === false)
                    {
                        descriptorsByCategory[templateList[i].groups[j]][templateList[i].descriptors[k].category] = [];
                    }

                    if (descriptorsByCategory[templateList[i].groups[j]][templateList[i].descriptors[k].category].includes(templateList[i].descriptors[k].value) === false)
                    {
                        descriptorsByCategory[templateList[i].groups[j]][templateList[i].descriptors[k].category].push(templateList[i].descriptors[k].value);
                    }
                }
            }
        }
...
function bMorphEligibleForCategory(group, category, morphDescriptorsByGroup, permutationAllowedDescriptorValuesForcategory)
{
    if ( morphDescriptorsByGroup[group] === undefined || morphDescriptorsByGroup[group][category] === undefined)
    {
        return false;
    }

    for (let i = 0; i < permutationAllowedDescriptorValuesForcategory.length; i++)
    {
        for (let j = 0; j < morphDescriptorsByGroup[group][category].length; j++)
        {
            if (permutationAllowedDescriptorValuesForcategory[i] === morphDescriptorsByGroup[group][category][j])
            {
                return true;
            }
        }
...
function bMorphIsValid(NPCrecordHandle, NPCinfo, morph, group, descriptorsByGroup, chosenPermutation, userForcedMorphs, allAssignedMorphs, allAssignedGroups, forwardedRequiredMorphs, failureModes, attributeCache, xelib, logMessage)
{
    ...
    if (userForcedMorphs.includes(morph.name) === false)
    {
        ...
        //required groups
        for (let i = 0; i < morph.requiredTemplates.length; i++)
        {
            // check if the required group is in the current combination
            if (allAssignedGroups.includes(morph.requiredTemplates[i].group))
            {
                // if yes, check if that group was already assigned and return false if not
                let requirementIndex = allAssignedGroups.indexOf(morph.requiredTemplates[i].group);
                let currentMorphIndex = allAssignedGroups.indexOf(group);
                if (requirementIndex < currentMorphIndex && allAssignedMorphs[requirementIndex] !== morph.requiredTemplates[i].name)
                {
                    Aux.updateFailureModes(failureModes, "Current morph required " + morph.requiredTemplates[i].name + " in group " + morph.requiredTemplates[i].group + " but this group had already been assigned to " + allAssignedMorphs[requirementIndex]);
                    return false;
                }
            }
        }
        // forwarded requirements
        for (let i = 0; i < forwardedRequiredMorphs.length; i++)
        {
            if (group === forwardedRequiredMorphs[i].group && morph.name !== forwardedRequiredMorphs[i].name)
            {
                Aux.updateFailureModes(failureModes, "Current morph required " + forwardedRequiredMorphs[i].name + " in group " + forwardedRequiredMorphs[i].group + " and this morph is not the one required.");
                return false;
            }
        }
    }

    // For version 1.9. the permutation compatibility check must be performed AFTER the NPC compatibility check. As soon as either check fails this function returns false, and the calling function needs to know for certain if the morph is discarded due to intrinsic incompatibility with the NPC (vs. due to incompatibility with the permutation).
    ...
    if (chosenPermutation !== undefined && chosenPermutation.allowedBodyGenDescriptors !== undefined) // note: don't respect userForcedMorphs here because if this returns false, PO.chooseRandomPermutation will try to find a different chosenPermutation to satisfy this function
    {
        ...
        {
            if (bMorphEligibleForCategory(group, category, descriptorsByGroup, chosenPermutation.allowedBodyGenDescriptors[category]) === true && bMorphDescriptorsIncludesCurrent(morph, category, chosenPermutation.allowedBodyGenDescriptors[category]) === false)
            {
...
function chooseRandomFromProbabilityWeighting(items)
{
    ...
    return Aux.chooseRandomFromArray(choiceList);
}

function filterMorphsByForceIf(NPCrecordHandle, NPCinfo, morphList, currentGroup, descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage)
{
    ...
    for (let i = 0; i < morphList.length; i++)
    {
        ...
        if (morphList[i].forceIfAttributes.length > 0)
        {
            ...
            {
                if (Aux.bAttributeMatched(morphList[i].forceIfAttributes[j][0], morphList[i].forceIfAttributes[j][1], NPCrecordHandle, logMessage, xelib, attributeCache) === true && bMorphIsValid(NPCrecordHandle, NPCinfo, morphList[i], currentGroup, descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, [], attributeCache, xelib, logMessage) === true)
                {
