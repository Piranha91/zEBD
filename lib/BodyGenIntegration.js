...
module.exports = function(Aux)
{
    ...
    BGI.testFunc = function(NPCrecordHandle, NPCinfo, morphAssignments, categorizedMorphs, chosenPermutation, attributeCache, xelib, logMessage)
    {
        let assignedGroups = morphAssignments.groups;
        let assignedMorphs = morphAssignments.morphs;
        let currentGroup = "";
        let currentMorphName = "";
        ...
        for (let i = 0; i < assignedMorphs.length; i++)
        {
            ...
            currentMorphName = assignedMorphs[i];
            morphListForGroup = categorizedMorphs[NPCinfo.gender][NPCinfo.race][currentGroup];

            for (let j = 0; j < morphListForGroup.length; j++)
            {
                if (morphListForGroup[j].name === currentMorphName)
                {
                    testMorph = morphListForGroup[j];
                    break;
                }
            }

            if (bMorphIsValid(NPCrecordHandle, NPCinfo, testMorph, morphListForGroup, chosenPermutation, [], failureModes, attributeCache, xelib, logMessage) === false)
            {
                logMessage("I fucked up.");
                return failureModes;
            }
        }
        return undefined;
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, attributeCache, logMessage, passMessage)
    {
        let returnObj = {};
        let assignedMorphs = [];
        let assignedGroups = [];
        ...
        morphs = categorizedMorphs[NPCinfo.gender][NPCinfo.race];
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                if (chosenMorph === undefined)
                {
                    ...
                    {
                        logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") was not found. Choosing at random.");
                    }
                    else
                    {
                        passMessage.message = "the consistency morph was not found."
                    }
                    break;
                }

                if (bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphs[assignedGroup], assignedAssetPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage) === false)
                {
                    bForcedValid = false;
                    if (bSuppressErrorMessage === false)
                    {
                        logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") is invalid according to current settings. Choosing at random.");
                        logMessage(Aux.formatFailureModeString_consistency(failureModes, "morph"));
                    }
                    else
                    {
                        //examine failureModes to determine what should be passed back
                        ...
                        {
                            passMessage.message = "the consistency morph's distribution rules were incompatible with the NPC's attributes.";
                        }
                        else if (NPCincompatibleKeys === 0 && permutationIncompatibleKeys > 0)
                        {
                            passMessage.message = "the consistency morph's descriptors were incompatible with the permutation's restrictions.";
                        }
                        else if (NPCincompatibleKeys > 0 && permutationIncompatibleKeys > 0)
                        {
                            passMessage.message = "the consistency morph's distribution rules were incompatible with the NPC's attributes and the consistency morph's descriptors were incompatible with the permutation's restrictions.";
                        }
                    }
                    break;
                }
            ...
            if (userForcedMorphs.length > 0 && userForcedMorphsConsistencyTracker.length > 0)
            {
                ...
                {
                    passMessage.message = "the consistency morph does not comply with your Specific NPC Assignments."
                }
            }
        ...
        //debug
                            ...
                            let permHasSpecial = false;
                            let assetHasSpecial = false;
            ...
            for (let i = 0; i < combination.length; i++)  // choose a morph for each item in the combination
            {
                ...
                while (bFoundValidMorph === false && selectionList.length > 0) // note: The tracked parameter here is selectionList
                {
                    ...
                    bFoundValidMorph = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphs[item], assignedAssetPermutation, userForcedMorphs, failureModesByCombination[item], attributeCache, xelib, logMessage);
                                    ...
                                    //debug
                                    if (assignedAssetPermutation !== undefined && assignedAssetPermutation.allowedBodyGenDescriptors !== undefined)
                                    {
                                        for (let [Acategory, Avalues] of Object.entries(assignedAssetPermutation.allowedBodyGenDescriptors))
                                        {
                                            for (let x = 0; x < Avalues.length; x++)
                                            {
                                                let Avalue = Avalues[x];
                                                if (Acategory === "chest" && Avalue === "busty")
                                                {
                                                    permHasSpecial = true;
                                                }
                                                for (let y = 0; y < chosenMorph.descriptors.length; y++)
                                                {
                                                    let Mcategory = chosenMorph.descriptors[y].category;
                                                    let Mvalue = chosenMorph.descriptors[y].value;

                                                    if (Acategory === Mcategory)
                                                    {
                                                    ...
                                                    if (Mcategory === "chest" && Mvalue === "busty")
                                                    {
                                                        assetHasSpecial = true;
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (permHasSpecial === true && assetHasSpecial === false)
                                    {
                                        let retest1 = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphs[item], assignedAssetPermutation, userForcedMorphs, failureModesByCombination[item], attributeCache, xelib, logMessage);
                                        let specialFlag
                                    }


                                    if (hasSameCategory === true && hasSameValue === false)
                                    {
                                        let retest2 = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphs[item], assignedAssetPermutation, userForcedMorphs, failureModesByCombination[item], attributeCache, xelib, logMessage);
                                        let generalFlag;
                                    }
        ...
        {            
            // reminder: consistency and linked NPC data are handled externally as of v1.8
            ...
            let debug = this.testFunc(NPCrecordHandle, NPCinfo, returnObj, categorizedMorphs, assignedAssetPermutation, attributeCache, xelib, logMessage)
            if (debug !== undefined)
            {
                logMessage("This is wrong.");
            }
