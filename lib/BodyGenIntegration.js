...
module.exports = function(Aux)
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, attributeCache, logMessage, passMessage)
    {
        let returnObj = {};
        let assignedMorphs = [];
        let assignedGroups = [];
        ...
        if (bLinkNPCsWithSameName === true && NPCinfo.isUnique === true)
        {
            ...
            if (linkedData !== undefined && linkedData.assignedMorphs.length > 0 && linkedData.assignedGroups.length > 0)
            {
                ...
                if (assignedMorphs !== undefined && assignedGroups.length > 0 && assignedMorphs.length > 0)
                {
                    bFoundValidCombination = true;
                    bLinkedMorphsLoaded = true;
                }
    ...
    function bMorphIsValid(NPCrecordHandle, NPCinfo, morph, morphList, chosenPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage)
    {
        ...
        if (!Number.isNaN(morph.weightRange[0]) && NPCinfo.weight < morph.weightRange[0])
        {
            Aux.updateFailureModes(failureModes, "NPC weight  <  " + morph.weightRange[0].toString());
            return false;
        }
        if (!Number.isNaN(morph.weightRange[1]) && NPCinfo.weight > morph.weightRange[1])
        {
            Aux.updateFailureModes(failureModes, "NPC weight  >  " + morph.weightRange[1].toString());
            return false;
        }
