...
module.exports = function(Aux)
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, attributeCache, logMessage)
    {
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                {
                    bForcedValid = false;
                    logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.formID + ") was not found. Choosing at random.");
                    break;
                }
                ...
                {
                    bForcedValid = false;
                    logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.formID + ") is invalid according to current settings. Choosing at random.");
            ...
            {
                logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.formID + ") does not comply with your Specific NPC Assignments.");
                bForcedValid = false;
            }
        ...
        else
        {
            ...
            {
                logMessage("\nBodyGen Assignment: no BodyGen morphs satisfied criteria for " + NPCinfo.name + " (" + NPCinfo.formID + "). Skipping this NPC.");
    ...
    function bMorphIsValid(NPCrecordHandle, NPCinfo, morph, morphList, chosenPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage)
    {
        ...
        if (morph.allowedAttributes.length > 0)
        {
            ...
            {
                failureModeString = "NPC lacks all of the following allowed attributes:" + tmpFailModes;
                if (failureModes[failureModeString] === undefined)
                {
                    failureModes[failureModeString] = 1;
                }
