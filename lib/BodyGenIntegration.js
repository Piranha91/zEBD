...
module.exports = function(Aux)
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, bForceConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, attributeCache, logMessage)
    {
        let returnObj = {};
        let assignedMorphs = [];
        let assignedGroups = [];
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            ...
            {
                logMessage("\nBodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.EDID + "/" + NPCinfo.formID + ") does not comply with your Specific NPC Assignments."); // this message should not be supressed ever
                bForcedValid = false;
            }
        ...
        if (bEnableConsistency === true && bForceConsistency === true && bFoundValidCombination === false)
        {
            return undefined;
        }
