...
module.exports = function(Aux, PO)
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, attributeCache, logMessage)
    {
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                if (bMorphIsValid(NPCrecordHandle, chosenMorph, assignedAssetPermutation, attributeCache, xelib, logMessage) === false)
                {
                    bForcedValid = false;
                    break;
                }
        ...
        while (bFoundValid === false && combinations.length > 0)
        {
            ...
            for (let i = 0; i < combination.length; i++)
            {
                ...
                {
                    morphs[item] = filterMorphsByForceIf(NPCrecordHandle, morphs[item], attributeCache, xelib, logMessage);
                ...
                if (bMorphIsValid(NPCrecordHandle, chosenMorph, assignedAssetPermutation, attributeCache, xelib, logMessage) === true)
                {
        ...
        if (bFoundValid === true)
        {
            if (bEnableConsistency === true)
            {
                ...
                updateBodyGenConsistencyRecord(consistencyRecords[NPCinfo.consistencyIndex], assignedMorphs);
            }   
    ...
    function filterMorphsByForceIf(NPCrecordHandle, morphList, attributeCache, xelib, logMessage)
    {
        ...
        for (let i = 0; i < morphList.length; i++)
        {
            ...
            if (morphList[i].forceIfAttributes.length > 0)
            {
                ...
                {
                    if (Aux.bAttributeMatched(morphList[i].forceIfAttributes[i][0], morphList[i].forceIfAttributes[i][1], NPCrecordHandle, logMessage, xelib, attributeCache) === true)
                    {
    ...
    function bMorphIsValid(NPCrecordHandle, morph, chosenPermutation, attributeCache, xelib, logMessage)
    {
        let bAAsatisfied = false;

        // check allowed BodyGen morphs from permutation
        if (chosenPermutation !== undefined && chosenPermutation.allowedBodyGenMorphs !== undefined && (chosenPermutation.allowedBodyGenMorphs.includes(morph.name) === false))
        {
            return false;
        }

        // check disallowed BodyGen morphs from permutation
        if (chosenPermutation !== undefined && chosenPermutation.disallowedBodyGenMorphs !== undefined && chosenPermutation.disallowedBodyGenMorphs.includes(morph.name) === true)
        {
            return false;
        }
    ...
    function updateBodyGenConsistencyRecord(assignmentRecord, chosenMorphs)
    {
