...
module.exports = function(Aux, PO)
{
    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, LinkedNPCNameExclusions, linkedNPCBodyGen, attributeCache, logMessage)
    {
        let returnObj = {};
        let assignedMorphs = [];
        let assignedGroups = [];
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                if (bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphListForGroup, assignedAssetPermutation, userForcedMorphs, attributeCache, xelib, logMessage) === false)
                {
                    bForcedValid = false;
            ...
            {
                logMessage("BodyGen Assignment: consistency morph for " + NPCinfo.name + " (" + NPCinfo.formID + ") does not comply with your Specific NPC Assignments. Choosing at random.");
                bForcedValid = false;
            }
        ...
        while (bFoundValidCombination === false && combinations.length > 0) // note: The tracked parameter here is combinations
        {
            ...
            for (let i = 0; i < combination.length; i++)  // choose a morph for each item in the combination
            {
                ...
                while (bFoundValidMorph === false && selectionList.length > 0) // note: The tracked parameter here is selectionList
                {
                    ...
                    bFoundValidMorph = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, morphs[item], assignedAssetPermutation, userForcedMorphs, attributeCache, xelib, logMessage);
        ...
        if (bFoundValidCombination === true)
        {
            ...
            Aux.updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, linkedData, LinkedNPCNameExclusions, linkedNPCBodyGen);
    ...
    function bMorphIsValid(NPCrecordHandle, NPCinfo, morph, morphList, chosenPermutation, userForcedMorphs, attributeCache, xelib, logMessage)
    {
