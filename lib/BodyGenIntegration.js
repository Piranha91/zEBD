    ...
    BGI.assignMorphs = function(NPCrecordHandle, BGconfig, categorizedMorphs, NPCinfo, bEnableConsistency, consistencyRecords, assignedAssetPermutation, userForcedAssignment, bLinkNPCsWithSameName, linkedNPCBodyGen, NPClinkGroup, bSuppressErrorMessage, aliasList, attributeCache, logMessage, passConsistencyMessage, passFailureMessage)
    {
        ...
        if (bEnableConsistency === true && NPCinfo.consistencyIndex > -1 && consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs !== undefined && bLinkedMorphsLoaded === false)
        {
            for (let i = 0; i < consistencyRecords[NPCinfo.consistencyIndex].assignedMorphs.length; i++)
            {
                ...
                if (bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, assignedGroup, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage) === false) // pass the original categorizedMorphs because morphs can be pruned upstream by 
                {
                    bForcedValid = false;
                    if (bSuppressErrorMessage === false)
                    {
        ...
        while (bFoundValidCombination === false && combinations.length > 0) // note: The tracked parameter here is combinations
        {
            ...
            for (let i = 0; i < combination.length; i++)  // choose a morph for each item in the combination
            {
                ...
                {
                    selectionList = filterMorphsByForceIf(NPCrecordHandle, NPCinfo, selectionList, item, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, failureModesByCombination[item], attributeCache, xelib, logMessage);
                ...
                while (bFoundValidMorph === false && selectionList.length > 0) // note: The tracked parameter here is selectionList
                {
                    ...
                    bFoundValidMorph = bMorphIsValid(NPCrecordHandle, NPCinfo, chosenMorph, item, BGconfig.descriptorsByGroup, assignedAssetPermutation, userForcedMorphs, failureModesByCombination[item], attributeCache, xelib, logMessage);
...
function bMorphIsValid(NPCrecordHandle, NPCinfo, morph, group, descriptorsByGroup, chosenPermutation, userForcedMorphs, failureModes, attributeCache, xelib, logMessage)
{
