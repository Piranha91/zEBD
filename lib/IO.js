...
module.exports = function(logDir, fh, modulePath)
    {
            ...
            for (let i = 0; i < files.length; i++)
            {
                if (files[i].split('.').pop().toLowerCase() === "json" && files[i].indexOf("TrimPaths") === 0) // only load files prefixed with "TrimPaths"
                {
                    try
                    {
                        ...
                        for (let j = 0; j < trimPathsFromFile.trimPaths.length; j++)
                        {
                            trimPathArray.push(trimPathsFromFile.trimPaths[j]);
                        }
        ...
        IO.loadBlockList = function(modulePath)
        {
            ...
            updateFormIDsFromPluginName(blockList.blockedNPCs, xelib)


            for (let i = 0; i < blockList.blockedPlugins.length; i++)
            {
                blockList.blockedPlugins[i] = blockList.blockedPlugins[i].trim();
            }

            return blockList;
        };

        IO.loadForceList = function(modulePath, assetPackSettings)
        {
            let path = modulePath + "\\NPC Configuration\\ForceNPCList.json";
            let forceList = fh.loadJsonFile(path);
            let bFound = false;
            let topLevelSubgroup = "";
            let tmpObj;

            if (forceList !== undefined)
            {
                updateFormIDsFromPluginName(forceList, xelib);
            }

            // find the top-level subgroups corresponding to each subgroup
            for (let i = 0; i < forceList.length; i++)
            {
                if (forceList[i].forcedSubgroups === undefined) // if the user only specified which asset pack to use and not which subgroup
                {
                    forceList[i].forcedSubgroups = []; 
                }
                for (let a = 0; a < assetPackSettings.length; a++)
                {
                    if (assetPackSettings[a].groupName === forceList[i].forcedAssetPack)
                    {
                        for (let j  = 0; j < forceList[i].forcedSubgroups.length; j++)
                        {
                            bFound = false;
                            for (let k = 0; k < assetPackSettings[a].subgroups.length; k++)
                            {
                                bFound = Aux.bSubgroupHasChildSubgroup(assetPackSettings[a].subgroups[k], forceList[i].forcedSubgroups[j]);
                                if (bFound === true)
                                {
                                    topLevelSubgroup = assetPackSettings[a].subgroups[k].id;
                                    break;
                                }
                            }
                            ...
                            {
                                tmpObj = {};
                                tmpObj.forcedSubgroupID = forceList[i].forcedSubgroups[j];
                                tmpObj.topLevelSubgroup = topLevelSubgroup;
                                forceList[i].forcedSubgroups[j] = tmpObj;
                            }
                            else
                            {
                                alert("Warning in ForceNPCList.json: Subgroup " + forceList[i].forcedSubgroups[j] + " was not found in " + forceList[i].forcedAssetPack + ". Ignoring.");
                                forceList[i].forcedSubgroups.splice(j, 1);
                            }
                        }
                    }
                }
        ...
        IO.loadDevControls = function(modulePath, settings)
        {
            let path = modulePath + "\\zEBD Assets\\MiscConfig\\DevControls.json";

            let devControls =  fh.loadJsonFile(path);

            if (devControls !== undefined)
            {
                if (devControls.verboseLogger !== undefined)
                {
                    settings.bVerboseMode = devControls.verboseLogger;
                }

                if (devControls.permutationBuildupLogger !== undefined)
                {
                    settings.permutationBuildUpLogger = devControls.permutationBuildupLogger;
                }

                if (devControls.savePermutations !== undefined)
                {
                    settings.savePermutations = devControls.savePermutations;
                }

                if (devControls.loadPermutations !== undefined)
                {
                    settings.loadPermutations = devControls.loadPermutations;
                }
            }
        };
        ...
        IO.generateBodyGenMorphs = function(assignments, templates, dataPath, patchFileName)
        {
            ...
            for (let formID in assignments)
            {
                ...
                for (let i = 0; i < assignment.presets.length; i++)
                {
                    writeString += assignment.presets[i];
                    if (i < assignment.presets.length - 1)
                    {
                        writeString += ",";
                    }
                }
                ...
                try
                    {
                        ...
                        {
                            alert("Set iScaleMode=2 and iBodyMorphMode=2. Make sure you have body morphs with zeroed sliders installed and you're good to go.");
                        }
                        else if (check1 === true)
                        {
                            alert("Could not find setting \"iBodyMorphMode\" in skee64.ini. Make sure you're using the latest version of RaceMenu.");
                        }
                        else if (check2 === true)
                        {
                            alert("Could not find setting \"iScaleMode\" in skee64.ini. Make sure you're using the latest version of RaceMenu.");
                        }
                        else
                        {
                            alert("Could not find setting \"iScaleMode\" or \"iBodyMorphMode\" in skee64.ini. Make sure you're using the latest version of RaceMenu.");
                        }
                    }
                    catch (e)
                    {
...
function updateFormIDsFromPluginName(assignmentList, xelib)
{    
    for (let i = 0; i < assignmentList.length; i++)
    {
        if (assignmentList[i].rootPlugin === undefined || assignmentList[i].rootPlugin.trim() === "")
        {
            // Get rid of the first two characters of the formID if user provided the full length formID
            if (assignmentList[i].formID !== undefined && assignmentList[i].formID.length === 8)
            {
                assignmentList[i].formID = assignmentList[i].formID.substring(2, 9);
            }
            // replace them with the Hex index of the rootPlugin
            if (assignmentList[i].formID !== undefined && assignmentList[i].formID.length === 6)
            {
                assignmentList[i].formID = findPluginHexLoadOrder(assignmentList[i].rootPlugin, xelib) + assignmentList[i].formID;  
            }
        }
    }
...
function findPluginHexLoadOrder(name, xelib)
{
    let fileNames = xelib.GetLoadedFileNames(true);
    let index;
    let bFound = false;
...
function validatesubgroupSettings(currentsubgroup, bParsedSuccessfully, packSettingsName, ErrorHandler, fh, pathWarnings, IDs, userKeywords, subgroupHierarchy)
{
    ...
    if (currentsubgroup.allowedBodyGenPresets === undefined) { currentsubgroup.allowedBodyGenPresets = []; }
    if (currentsubgroup.disalloweBodyGenPresets === undefined) { currentsubgroup.disallowedBodyGenPresets = []; }
