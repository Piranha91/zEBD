...
module.exports = function(logDir, fh, modulePath)
    {
        ...
        IO.deleteConsistency = function(modulePath, mode)
        {
            ...
            else
            {
                ...
                this.saveConsistency(modulePath, consistency);
            }     
        };
        ...
        IO.deleteSavedPermutationsRecords = function(modulePath)
        {
            ...
            let path = modulePath + "\\NPC Configuration\\GeneratedPermutations_Records.json";
            if (fh.jetpack.exists(path))
            {
                fh.jetpack.remove(path);
                alert("Deleted saved permutations and records at:\n" + path);
            }
            else
            {
        ...
        IO.saveGeneratedPermutations_Records = function(modulePath, permutations, recordTemplates, linkageList, maxPriority)
        {
            let path = modulePath + "\\NPC Configuration\\GeneratedPermutations_Records.json";
            let saveObj = {};
            saveObj.permutations = permutations;
            saveObj.recordTemplates = recordTemplates;
            saveObj.linkageList = linkageList;
            saveObj.maxPriority = maxPriority;
            fh.saveJsonFile(path, saveObj);
        }

        IO.loadGeneratedPermutations_Records = function(modulePath, permutations, recordTemplates)
        {
            let path = modulePath + "\\NPC Configuration\\GeneratedPermutations_Records.json";

            let loadObj = fh.loadJsonFile(path);

            if (loadObj.permutations === undefined || loadObj.permutations.length === 0)
            {
                alert("No permutations could be loaded from " + path + ". Generating permutations from settings instead.");
                return -1;
            }

            if (loadObj.recordTemplates === undefined || loadObj.recordTemplates.length === 0)
            {
                alert("No records could be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }

            if (loadObj.linkageList === undefined || loadObj.linkageList.length === 0)
            {
                alert("Linkages could not be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }
            else
            {
                let elementName;
                let indexInList;
                for (let i = 0; i < loadObj.linkageList.length; i++)
                {
                    for (let j = 0; j < loadObj.linkageList[i].length; j++)
                    {
                        elementName = loadObj.linkageList[i][j][0];
                        indexInList = loadObj.linkageList[i][j][1];
                        placeRecordAtPath(elementName, loadObj.recordTemplates[i], loadObj.recordTemplates[indexInList]);
                    }
                }
            }

            if (loadObj.maxPriority === undefined || loadObj.maxPriority < 0)
            {
                alert("Max priority could be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }

            permutations = loadObj.permutations;
            recordTemplates = loadObj.recordTemplates;

            return loadObj.maxPriority;
        }
            ...
            else
            {
                ...
                for (let i = 0; i < linkageList.length; i++)
                {
                    for (let j = 0; j < linkageList[i].length; j++)
                    {
                        ...
                        placeRecordAtPath(elementName, recordTemplates[i], recordTemplates[indexInList]);
                    }
                }
            }
...
function getLastarrayPos(str)
{
     // get all bracket positions
     let indexBrackets = str.match(/\[.*?\]/g);
     let lastIndexBracket = indexBrackets[indexBrackets.length - 1];
     let lastIndexBracketPos = str.lastIndexOf(lastIndexBracket);
     let prefix_path = trimSlashes(str.substring(0, lastIndexBracketPos));
     let sPosition = lastIndexBracket.substring(1, lastIndexBracket.length - 1);
     let position = parseInt(sPosition);
     let suffix_path = trimSlashes(str.substring(lastIndexBracketPos + lastIndexBracket.length, str.length));
     
     return [prefix_path, position, suffix_path];
}

function placeRecordAtPath (path, recipientObject, recordToPlace)
{
    let pSplit = path.split('\\');
    subPath = path.substring(pSplit[0].length + 1, path.length);

    // if there is only one array element in pSplit, assign recordToPlace to that element of recipientObject
    if (pSplit.length === 1)
    {
        recipientObject[pSplit[0]] = recordToPlace;
    }

    else
    {
        // check if the next pSplit entry is an array
        let arrIndex = getArrayIndexFromBrackets(pSplit[1]);
        ...
        if (arrIndex >= 0) // if array
        {
            ...
            // check if the recordToPlace is to be assigned directly into the recipientObject at this array index, or if it goes into a sub-value
            if (subPath === "")
            {
                recipientObject[pSplit[0]][arrIndex] = recordToPlace;
            }
            else
            {
                placeRecordAtPath(subPath, recipientObject[pSplit[0]][arrIndex], recordToPlace);
            }
        }
        else // if object
        {
            placeRecordAtPath(subPath, recipientObject[pSplit[0]], recordToPlace);
        }
    }   
}

/* This version of the function directly assigned the object rather than assigning a subproperty of the object. Javascript does not allow assigning a value by reference (wtf) but does allow assigning subproperties by reference (wtf) so need to rewrite 
function placeRecordAtPath (path, recipientObject, recordToPlace)
{
    let pSplit = path.split('\\');
    ...
    if (path.length === 0)
    {
        recipientObject = recordToPlace;
        return;
    }
    else
    {
        subPath = path.substring(pSplit[0].length + 1, path.length);
        // check if there are two or more pSplit entries after the current one
        if (pSplit.length > 1)
        {
            // check if the next pSplit entry is an array
            let arrIndex = getArrayIndexFromBrackets(pSplit[1]);
        ...
        // if not, go into the current path address (note that the length can never be 0, and if the path is "" it's already captured above and returned)
        else
        {
            placeRecordAtPath(subPath, recipientObject[pSplit[0]], recordToPlace);
        }
...
function getArrayIndexFromBrackets(str)
{
    str = str.substring(1, str.length);
    str = str.substring(0, str.length - 1);

    let index = parseInt(str);
    if (Aux.isValidNumber(index) === false)
    {
        return -1;
    }
    else
...
/* This function converts a path like   aaaaa[1]bbbbb[2]cccc into array of arrays [ ["aaaaa", 1, ""], ["bbbbb", 2, "ccccc"] ]
// realized it's not necessary to achieve what I need (because for linking records I only need the last array position), but keeping it here for future reference
function getArrayIndicesFromStringPath(str)
{
    let buildSubstring = "";
    let subtractString = str;
    let currentIndex = -1;
    let trailingSubstring = "";
    let arrays = [];
    // get all bracket positions
    let indexBrackets = str.match(/\[.*?\]/g);
    
    if (indexBrackets !== undefined && indexBrackets !== null)
    {
        for (let i = 0; i < indexBrackets.length; i++)
        {
            buildSubstring += subtractString.substring(0, subtractString.indexOf(indexBrackets[i]));
            trailingSubstring = "";
            currentIndex = parseInt(indexBrackets[i].substring(1, indexBrackets[i].length - 1));

            if (Number.isNaN(currentIndex) === false)
            {
                buildSubstring = trimSlashes(buildSubstring);
                trailingSubstring = subtractString.substring(buildSubstring.length + indexBrackets[i].length + 1, subtractString.length);
                trailingSubstring = trimSlashes(trailingSubstring);

                // store the array
                arrays.push([buildSubstring, currentIndex, trailingSubstring]);
                subtractString = subtractString.substring(buildSubstring.length, subtractString.length - 1);
            }
        }
    }
...
function getStringArrayIndex(str)
{
    let indexBrackets = str.match(/\[.*?\]/g);
    if (indexBrackets === null)
    {
        return -1;
    }
    let lastIndexBracket = indexBrackets[indexBrackets.length - 1];
