...
module.exports = function(logDir, fh, modulePath)
    {
        ...
        IO.loadSelectedBodyGenMorphs = function(forceNPClist, NPClist, templateList)
        {
            ...
            for (let i = 0; i < split.length; i++)
            {
                ...
                if (bFoundInForceList === false)
                {
                    ...
                    {
                        if (NPClist[j].formID.substring(2, 9) === formID && NPClist[j].masterRecordFile === rootPlugin)
                        {
        ...
        IO.validatePackSettings = function(currentPackSettings, bParsedSuccessfully, pathWarnings, userKeywords)
        {
