...
module.exports = function(logDir, fh, modulePath)
    {
        ...
        IO.loadGeneratedPermutations_Records = function(modulePath, permutations, recordTemplates, logMessage)
        {
            let path = modulePath + "\\NPC Configuration\\GeneratedPermutations_Records.json";
            ...
            {
                logMessage("No saved permutations/records were found at " + path + ". Generating permutations from settings instead.");
                return -1;
            }

            if (loadObj.permutations === undefined || loadObj.permutations.length === 0)
            {
                logMessage("No permutations could be loaded from " + path + ". Generating permutations from settings instead.");
                return -1;
            }

            if (loadObj.recordTemplates === undefined || loadObj.recordTemplates.length === 0)
            {
                logMessage("No records could be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }

            if (loadObj.linkageList === undefined || loadObj.linkageList.length === 0)
            {
                logMessage("Linkages could not be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }
            else
            {
            ...
            {
                logMessage("Max priority could be loaded from " + path + ". Generating records from settings instead.");
                return -1;
            }

            // copy loadObj values to the arrays passed by reference (can't use = bacause js breaks the memory assignment)
            permutations.push(...loadObj.permutations);
            recordTemplates.push(...loadObj.recordTemplates);
