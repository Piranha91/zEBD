...
module.exports = function(logDir, fh, modulePath)
    {
        ...
        IO.generatePermutationLog = function (permutationHolders, logDir, categorizedTemplateList, bLogAssignedOnly)
        {
            ...
            let pathArray = [];
            let linkedRecords = [];
            let linkedRecordSubset = [];

            for (let i = 0; i < permutationHolders.length; i++)
            {
                if ((permutationHolders[i].NPCsAssignedTo === undefined || permutationHolders[i].NPCsAssignedTo.length === 0) && bLogAssignedOnly === true)
                {
                    continue;
                }
                ...
                if (permutationHolders[i].templates != undefined && permutationHolders[i].templates.length > 0)
                {
                    ...
                    for (let j = 0; j < permutationHolders[i].templates.length; j++)
                    {
                        // get subRecords
                        linkedRecordSubset = [];
                        collectZEBDLinkToRecords(permutationHolders[i].templates[j], linkedRecordSubset, categorizedTemplateList);
                        linkedRecords.unshift(...linkedRecordSubset);
                    }

                    for (let k = 0; k < linkedRecords.length; k++)
                    {
                        writeString += "\t\t" + linkedRecords[k].EDID + " [" + linkedRecords[k].zEBDSignature + "]: " + linkedRecords[k].zEBDformID + "\n";
                    }
            ...
            else
            {
                ...
                for (let i = 0; i < loadObj.linkageList.length; i++)
                {
                    for (let j = 0; j < loadObj.linkageList[i].length; j++)
                    {
                        ...
                        Aux.assignValueToObjectPath(elementName, loadObj.recordTemplates[i], loadObj.recordTemplates[indexInList]);
                    }
                }
            }
