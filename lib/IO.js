...
module.exports = function(logDir, fh, modulePath)
    {
        ...
        IO.loadBodyGenConfig = function(loadPath)
        {
            ...
            for (let i = 0; i < cfg.templates.length; i++)
            {
                ...
                // v 1.9h update: assign blank RequiredTemplates to templates from previous versions
                if (cfg.templates[i].requiredTemplates === undefined)
                {
                    cfg.templates[i].requiredTemplates = [];
                }
            }
            ...
            {
                cfg.templateGroups = cfg.templateGroups.sort();
            }
            ...
            {
                cfg.templateDescriptors = cfg.templateDescriptors.sort();
            }
                ...
                if (obj.templates !== undefined)
                {
                    for (let i = 0; i < obj.templates.length; i++)
                    {
                        ...
                        for (let j = 0; j < currentBodyGenConfig.templates.length; j++)
                        {
                            ...
                            // new in v1.9h: notes
                            if (currentBodyGenConfig.templates[j].notes === undefined)
                            {
                                currentBodyGenConfig.templates[j].notes = "";
                            }
                        }
        ...
        IO.loadRecordTemplates = function(loadPath, modulePath, groupDefs, logMessage)
        {
            let path_RecordTemplates = loadPath + "\\zEBD Assets\\RecordTemplates";
            if (fh.jetpack.exists(path_RecordTemplates) === false) //if there are no record template at loadPath, try modulePath
            {
                path_RecordTemplates = modulePath + "\\zEBD Assets\\RecordTemplates";
            }
        ...
        IO.loadExtraAAexclusions = function(modulePath)
        {
            let path = modulePath + "\\zEBD Assets\\MiscConfig\\extraAAexclusions.json";
            let exclusions = [];
