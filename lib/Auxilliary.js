module.exports =
    {
        ...
        generateFormIDsignature(formID)
        {
            return "xx" + formID.substring(2, 9);
        },
        ...
        {
            if (flattenedSubgroups === undefined)
            {
                return "NO SUBGROUPS AVAILABLE"; // only reached under specific circumstances involving conflicts between permutation and bodygen
            }
        ...
        formatAllowedBodyGenForVerbose(ABG)
        {
            let str = "";

            if (Object.keys(ABG).length === 0)
            {
                str += "Current permutation has no stipulations for allowed BodyGen.\n";
            }
            else
            {
                str += "Current permutation has the following allowed BodyGen:\n";
                for (let [category, values] of Object.entries(ABG))
                {
                    str += category + ": " + values.join(", ") + "\n";
                }
            }
            return str;
        },

        formatDisallowedBodyGenForVerbose(DBG)
        {
            let str = "";
            if (DBG.length === 0)
            {
                str += "Current permutation has no stipulations for disallowed BodyGen.\n";
            }
            else
            {
                str += "Current permutation has the following stipulations for disallowed BodyGen:\n";
                // collect descriptors into categories
                let desc = {};
                for (let i = 0; i < DBG.length; i++)
                {
                    if (desc[DBG[i].category] === undefined)
                    {
                        desc[DBG[i].category] = [];
                    }
                    desc[DBG[i].category].push(DBG[i].value);
                }
                // log the descriptors
                for (let [category, values] of Object.entries(desc))
                {
                    str += category + ": " + values.join(", ") + "\n";
                }
            }
        ...
        generateDateString()
        {
            ...
            let month = this.padDate(currentDate.getMonth() + 1); // getMonth() is 0-indexed
            let day = this.padDate(currentDate.getDate());
            let hours = this.padDate(currentDate.getHours());
            let minutes = this.padDate(currentDate.getMinutes());
        ...
        createConsistencyRecord(NPCrecordHandle, NPCinfo, consistencyRecords, xelib)
        {
            ...
            assignmentRecord.formIDSignature = this.generateFormIDsignature(NPCinfo.formID);
        ...
        getSubgroupTopLevelIndex: function(subgroupID, subgroupHierarchy) // looks recursively through top-level subgroups to find a child subgroup's top-level index
        {
            for (let i =0; i < subgroupHierarchy.length; i++)
            {
                if (this.bSubgroupInCurrentSubgroup(subgroupHierarchy[i], subgroupID) === true)
                {
                    return i;
                }
            }
        ...
        bSubgroupInCurrentSubgroup: function(currentSubgroup, id)
        {
            let bFound = false;
            ...
            else if (currentSubgroup.subgroups.length > 0)
            {
                ...
                {
                    bFound = this.bSubgroupInCurrentSubgroup(currentSubgroup.subgroups[i], id)
        ...
        getAssetPackIndexByName: function(name, assetPacks)
        {
            for (let i = 0; i < assetPacks.length; i++)
            {
        ...
        getPermutationSubgroupIDs: function(permutation)
        {
            let arr = [];
        ...
        convertAttributePairsToObjects: function(attributePairs, sourceSubgroup)
        {
            let Aobj;
            let objectList = [];
            for (let i = 0; i < attributePairs.length; i++)
        ...
        logVerbose: function(logString, verboseReport, out_writeVerbose)
        {
