...
module.exports =
    {
        ...
        updatelinkedNPCdata(bLinkNPCsWithSameName, NPCinfo, linkedData, exclusionList, linkedNPCdata)
        {
            let bExcluded = false;
            if (bLinkNPCsWithSameName === true && (NPCinfo.isUnique === true || (NPCinfo.name === "Ulfric Stormcloak" || NPCinfo.name === "General Tullius"))) // CWBattleUlfric/Tullius are for some reason not flagged as unique
            {
                ...
                if (bExcluded === false)
                {
                    linkedNPCdata.push([NPCinfo.name, NPCinfo.race, NPCinfo.gender, linkedData]);
                }
            }
        },
