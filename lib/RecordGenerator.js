...
module.exports = function(logDir, fh)
{
    ...
    RG.generateRecords = function (permutations, settings, recordTemplates, assetPackSettings, helpers)
    {
        ...
        mapRecordsToUniques(filledTemplateList, categorizedTemplateList, RG.maxPriority); // makes all recordTemplates with the same values point to the same address in memory, so that when one of them gets a formID assigned it applies to all
...
function populatePermutationPaths (permutation, recordTemplates, filledTemplateList, recordToPathMap, logMessage)
{
    ...
    for (let i = 0; i < permutation.templates.length; i++)
    {
        collectGeneratedzEBDrecords(permutation.templates[i], filledTemplateList[permutation.sourceAssetPack], permutation.sourceAssetPack, i);
    }
...
function collectGeneratedzEBDrecords(record, storageList, zEBDsourceAssetPack, index)
{
    ...
    if (record.zEBDUniqueID != undefined)
    {
        ...
        storageList.push(record); // new in 1.8. Not sure why I was using angular.copy before, but it's more efficient to directly push the record because then it's directly linked in memory between the filled 1D record list and the permutation's record list
    }
    ...
    if (Aux.isObject(record) === true)
    {
        for (let [attribute, value] of Object.entries(record)) // iterate through each attribute of the record
        {
            ...
            if (Array.isArray(value))
            {
                ...
                {
                    collectGeneratedzEBDrecords(value[j], storageList, zEBDsourceAssetPack, index); // recursively check each element to see if it is a template record
                }
            }
            // if the attribute is not an array, recursively check to see if it is a template record
            else if (value != null)
            {
                collectGeneratedzEBDrecords(value, storageList, zEBDsourceAssetPack, index);
            }
        }
    }
    return record;
...
function mapRecordsToUniques(filledTemplateList, categorizedTemplateList, maxPriority)
{
    let uniques = [];
    let matched = false;
    ...
    {
        uniqueListBySource[sourceAssetPack] = []; // for linking subrecords
...
function linkUniqueSubRecords(obj, categorizedTemplateList, sourceAssetPack, debugObj) //
{
    ...
    for (let [attribute, value] of Object.entries(obj))
    {
        ...
        {
            //value = linkRecordToList(value, categorizedTemplateList[value.zEBDUniqueID][sourceAssetPack], debugParentObj);
            ...
            //linkUniqueSubRecords(value, categorizedTemplateList, sourceAssetPack, debugParentObj);
        }
    }
}

function getRecordIndexInList(record, list, debugObj) // get rid of debugParentObj when finisehd debugging
{
