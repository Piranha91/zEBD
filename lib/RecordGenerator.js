...
function assignRecordPathByMap(record, pathMap, assignmentValue)
{
    ...
    {
        Aux.assignValueToObjectPath(pathMapsForCurrent[i], record, targetPath);
        bPathAssigned = true;
    }
...
function bRecordPathsEquivalent(recA, recB, pathMapByRecordType)
{
    ...
    else
    {
        ...
        {
            if (Aux.getValueAtObjectPath(pathMapByRecordType[recA.zEBDUniqueID][i], recA) !== Aux.getValueAtObjectPath(pathMapByRecordType[recB.zEBDUniqueID][i], recB))
            {
                return false;
            }
        }
