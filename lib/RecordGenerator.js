        ...
        for (let [attribute, value] of Object.entries(record)) // iterate through each attribute of the record
        {
            if (Array.isArray(value)) // if the attribute is an array, iterate through its elements
            {
                for (let i = 0; i < value.length; i++)
                {
                    ...
                    {
                        linkUniqueRecord(value[i], uniqueRecordList); // recursively link the recordTemplate's child recordTemplates
                    }
                }
            }
            ...
            else
            {
                ...
                {
                    linkUniqueRecord(value, uniqueRecordList); // recursively link the recordTemplate's child recordTemplates
                }
            }
        }
    }
