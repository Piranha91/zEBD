...
function populatePermutationPaths (permutation, recordTemplates, fh, filledTemplateList, excludedExtensions, logMessage)
{
    ...
    {
        collectGeneratedzEBDrecords(permutation.templates[i], filledTemplateList, permutation.assetPackSettingsGeneratedFrom);
    }
...
function collectGeneratedzEBDrecords(record, storageList, assetPackSettingsGeneratedFrom)
{
    // check if the record is an zEBDrecord
    if (record.zEBDUniqueID != undefined)
    {
        record.zEBDassetPackSource = assetPackSettingsGeneratedFrom;
    ...
    if (Aux.isObject(record) === true)
    {
        for (let [attribute, value] of Object.entries(record)) // iterate through each attribute of the record
        {
            ...
            if (Array.isArray(value))
            {
                for (let j = 0; j < value.length; j++)
                {
                    collectGeneratedzEBDrecords(value[j], storageList, assetPackSettingsGeneratedFrom); // recursively check each element to see if it is a template record
                }
            }
            // if the attribute is not an array, recursively check to see if it is a template record
            else if (value != null)
            {
                collectGeneratedzEBDrecords(value, storageList, assetPackSettingsGeneratedFrom);
            }
        }
    }
    return record;
...
function getUniqueRecordList(recordList)
{
    ...
    for (let i = 0; i < recordList.length; i++)
    {
        ...
        {
            if (recordList[i].zEBDassetPackSource === recordList[i].zEBDassetPackSource && recordList[i].zEBDSignature === uniques[j].zEBDSignature && angular.equals(recordList[i], uniques[j]) === true) // check zEBDassetPackSource and record type before angular.equals to speed up algorithm
            {
                matched = true;
                break;
            }
...
function linkUniqueRecords(uniqueRecordList)
{
    ...
    {
        linkages.push(linkUniqueRecord(uniqueRecordList[i], uniqueRecordList, "", false, uniqueRecordList[i].zEBDassetPackSource));
    }
...
function linkUniqueRecord(record, uniqueRecordList, prefix, bStopRecording, assetPackSettingsGeneratedFrom)
{
    let currentLinkage = [];
    let oPrefix = prefix;
        ...
        for (let [attribute, value] of Object.entries(record)) // iterate through each attribute of the record
        {
            if (Array.isArray(value)) // if the attribute is an array, iterate through its elements
            {
                for (let i = 0; i < value.length; i++)
                {
                    ...
                    if (value[i].zEBDUniqueID != undefined) // if the array element is a child recordTemplate:
                    {
                        ...
                        {
                            if (uniqueRecordList[j].zEBDassetPackSource === assetPackSettingsGeneratedFrom && uniqueRecordList[j].zEBDSignature === value.zEBDSignature && angular.equals(value[i], uniqueRecordList[j]) === true) // check zEBDassetPackSource and record type before angular.equals to speed up algorithm
                            {
                                record[attribute][i] = uniqueRecordList[j]; // link the recordTemplate (array element) to uniqueRecordList
                                
                                if (bStopRecording === false)
                                {
                                    currentLinkage.push([prefix, j]); // attribute is already incorporated into prefix (along with the array position).
                                }

                                currentLinkage.push(...linkUniqueRecord(record[attribute][i], uniqueRecordList, prefix + "\\", true, assetPackSettingsGeneratedFrom)); // recursively link the recordTemplate's child recordTemplates
                            }
                        }
                    }
                    else if (Aux.isObject(value[i])) // if the array element is not a child recordTemplate but is an object:
                    {
                        currentLinkage.push(...linkUniqueRecord(value[i], uniqueRecordList, prefix + "\\", bStopRecording, assetPackSettingsGeneratedFrom)); // recursively link the recordTemplate's child recordTemplates
                    }
            ...
            else
            {
                ...
                if (value != null && value["zEBDUniqueID"] != undefined)
                {
                    for (let j = 0; j < uniqueRecordList.length; j++)
                    {
                        if (uniqueRecordList[j].zEBDassetPackSource === assetPackSettingsGeneratedFrom && uniqueRecordList[j].zEBDSignature === value.zEBDSignature && angular.equals(value, uniqueRecordList[j]) === true) // check zEBDassetPackSource and record type before angular.equals to speed up algorithm
                        {
                            record[attribute] = uniqueRecordList[j]; // link the recordTemplate (attribute) to uniqueRecordList

                            if (bStopRecording === false)
                            {
                                currentLinkage.push([prefix + attribute, j]);
                            }
                            currentLinkage.push(...linkUniqueRecord(record[attribute], uniqueRecordList, prefix, true, assetPackSettingsGeneratedFrom)); // recursively link the recordTemplate's child recordTemplates
                        }
                    }
                }
                else if (Aux.isObject(value)) // if the array element is not a child recordTemplate but is an object:
                {
                    ...
                    currentLinkage.push(...linkUniqueRecord(value, uniqueRecordList, prefix, bStopRecording, assetPackSettingsGeneratedFrom)); // recursively link the recordTemplate's child recordTemplates
                    prefix = oPrefix; // revert to avert "contaminating" other [attribute, value] pairs that get identified as zEBDrecords (if (value != null && value["zEBDUniqueID"] != undefined))
                }
            }
        }
