...
module.exports = function(logDir, fh)
{
    ...
    RG.generateRecords = function (permutations, settings, recordTemplates, assetPackSettings, helpers)
    {
        ...
        let categorizedTemplateList = initializeCategorizedTemplateList(assetPackSettings, recordTemplates); // generate a list of recordTemplates sorted by their source Asset Pack and sub-sorted by their EDID. Values used to make optimized sub-lists for decreasing search times during record linkage
...
function initializeCategorizedTemplateList(assetPackSettings, recordTemplates)
{
    ...
    for (let i = 0; i < recordTemplates.length; i++)
    {
        getAllRecordTemplateEDIDs(recordTemplates[i], EDIDlist);
    }
...
function getAllRecordTemplateEDIDs(object, storageList)
{
    if (object.EDID !== undefined)
    {
        storageList.push(object.EDID);
    }
    ...
    for (let [attribute, value] of Object.entries(object))
    {
        if (Array.isArray(value))
        {
            for (let i = 0; i < value.length; i++)
            {
                ...
                {
                    getAllRecordTemplateEDIDs(value[i], storageList);
                }
            }
        }
        ...
        {
            getAllRecordTemplateEDIDs(value, storageList);
        }
    }
}