...
function linkUniqueRecords(uniqueRecordList)
{
    ...
    {
        linkages.push(linkUniqueRecord(uniqueRecordList[i], uniqueRecordList, "", false));
    }
...
function linkUniqueRecord(record, uniqueRecordList, prefix, bStopRecording)
{
    let currentLinkage = [];
    let oPrefix = prefix;
        ...
        for (let [attribute, value] of Object.entries(record)) // iterate through each attribute of the record
        {
                    ...
                    if (value[i].zEBDUniqueID != undefined) // if the array element is a child recordTemplate:
                    {
                        ...
                        for (let j = 0; j < uniqueRecordList.length; j++)
                        {
                            if (angular.equals(value[i], uniqueRecordList[j]) === true)
                            {
                                ...
                                currentLinkage.push(...linkUniqueRecord(record[attribute][i], uniqueRecordList, prefix + "\\", true)); // recursively link the recordTemplate's child recordTemplates
                            }
                        }
                    }
                    else if (Aux.isObject(value[i])) // if the array element is not a child recordTemplate but is an object:
                    {
                        currentLinkage.push(...linkUniqueRecord(value[i], uniqueRecordList, prefix + "\\", bStopRecording)); // recursively link the recordTemplate's child recordTemplates
                    }
            ...
            else
            {
                ...
                if (value != null && value["zEBDUniqueID"] != undefined)
                {
                    for (let j = 0; j < uniqueRecordList.length; j++)
                    {
                        if (angular.equals(value, uniqueRecordList[j]) === true)
                        {
                            ...
                            currentLinkage.push(...linkUniqueRecord(record[attribute], uniqueRecordList, prefix, true)); // recursively link the recordTemplate's child recordTemplates
                        }
                    }
                }
                else if (Aux.isObject(value)) // if the array element is not a child recordTemplate but is an object:
                {
                    ...
                    currentLinkage.push(...linkUniqueRecord(value, uniqueRecordList, prefix, bStopRecording)); // recursively link the recordTemplate's child recordTemplates
                    prefix = oPrefix; // revert to avert "contaminating" other [attribute, value] pairs that get identified as zEBDrecords (if (value != null && value["zEBDUniqueID"] != undefined))
                }
            }
        }
